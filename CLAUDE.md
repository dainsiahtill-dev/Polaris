...
当前已迁入 runtime executable binding 的规则包括 `deterministic_typescript_return_object_semicolon_repair`、`deterministic_typescript_nullable_canvas_context_repair`、`deterministic_typescript_duplicate_object_property_repair`、`deterministic_typescript_enum_member_separator_repair`、`deterministic_go_bare_import_string_repair`、`deterministic_java_accessor_alias_repair`、`deterministic_cpp_include_path_repair`、`deterministic_cpp_standard_include_repair`、`deterministic_cpp_missing_private_members_repair`、`deterministic_cpp_placeholder_declaration_repair`、`deterministic_cpp_struct_getter_field_access_repair`、`deterministic_patch_residue_cleanup`。后续 Agent 不得把这些规则重新接回 legacy direct-write helper；C++ quote include path 修复必须通过 `RunDirectorRepairCommandV1(source_tool="deterministic_cpp_include_path_repair")`，C++ standard include 修复必须通过 `RunDirectorRepairCommandV1(source_tool="deterministic_cpp_standard_include_repair")`，C++ missing private members 修复必须通过 `RunDirectorRepairCommandV1(source_tool="deterministic_cpp_missing_private_members_repair")`，C++ placeholder declaration 修复必须通过 `RunDirectorRepairCommandV1(source_tool="deterministic_cpp_placeholder_declaration_repair")`，C++ struct getter field-access 修复必须通过 `RunDirectorRepairCommandV1(source_tool="deterministic_cpp_struct_getter_field_access_repair")`，TypeScript nullable canvas/context 修复必须通过 `RunDirectorRepairCommandV1(source_tool="deterministic_typescript_nullable_canvas_context_repair")`，TypeScript duplicate object property 修复必须通过 `RunDirectorRepairCommandV1(source_tool="deterministic_typescript_duplicate_object_property_repair")`，TypeScript enum member separator 修复必须通过 `RunDirectorRepairCommandV1(source_tool="deterministic_typescript_enum_member_separator_repair")`，patch residue cleanup 必须通过 `RunDirectorRepairCommandV1(source_tool="deterministic_patch_residue_cleanup")`。
