...
function parseArgs(argv) {
  const options = {
    ...
    taskFile: "",
    evidenceJson: "",
    round: parseIntArg(process.env.POLARIS_COMPUTER_USE_ROUND, 1, "POLARIS_COMPUTER_USE_ROUND"),
    model: String(process.env.POLARIS_COMPUTER_USE_MODEL || "computer-use-preview").trim(),
    displayWidth: parseIntArg(process.env.POLARIS_COMPUTER_USE_DISPLAY_WIDTH, 1280, "POLARIS_COMPUTER_USE_DISPLAY_WIDTH"),
    displayHeight: parseIntArg(process.env.POLARIS_COMPUTER_USE_DISPLAY_HEIGHT, 800, "POLARIS_COMPUTER_USE_DISPLAY_HEIGHT"),
    environment: String(process.env.POLARIS_COMPUTER_USE_ENVIRONMENT || "browser").trim(),
    maxSteps: parseIntArg(process.env.POLARIS_COMPUTER_USE_MAX_STEPS, 24, "POLARIS_COMPUTER_USE_MAX_STEPS"),
    timeoutMs: parseIntArg(process.env.POLARIS_COMPUTER_USE_TIMEOUT_MS, 20 * 60 * 1000, "POLARIS_COMPUTER_USE_TIMEOUT_MS"),
    actionDelayMs: parseIntArg(process.env.POLARIS_COMPUTER_USE_ACTION_DELAY_MS, 350, "POLARIS_COMPUTER_USE_ACTION_DELAY_MS"),
    requestTimeoutMs: parseIntArg(process.env.POLARIS_COMPUTER_USE_REQUEST_TIMEOUT_MS, 90 * 1000, "POLARIS_COMPUTER_USE_REQUEST_TIMEOUT_MS"),
    startUrl: String(process.env.POLARIS_COMPUTER_USE_START_URL || process.env.POLARIS_DEV_SERVER_URL || "http://127.0.0.1:5173").trim(),
    verifyCommand: String(process.env.POLARIS_COMPUTER_USE_VERIFY_CMD || "").trim(),
    openaiBaseUrl: String(process.env.OPENAI_BASE_URL || "https://api.openai.com/v1").trim(),
    chatCompatOn404: parseBool(process.env.POLARIS_COMPUTER_USE_CHAT_COMPAT, true),
    outputType: String(process.env.POLARIS_COMPUTER_USE_OUTPUT_TYPE || "input_image").trim(),
    reasonSummary: String(process.env.POLARIS_COMPUTER_USE_REASONING_SUMMARY || "concise").trim(),
    headless: parseBool(process.env.POLARIS_COMPUTER_USE_HEADLESS, true),
    acknowledgeSafety: parseBool(process.env.POLARIS_COMPUTER_USE_ACK_SAFETY, true),
    dryRun: false,
    outputJson: "",
  };
