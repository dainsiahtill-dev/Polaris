...
多轮确定性修复必须使用 repair kernel scheduler 建模 `priority`、`depends_on`、`round_number`、`max_rounds` 与 cycle breaker；receipt 必须携带 post-check evidence，证明对应 diagnostic 是否消失。迁移旧策略必须先用 `compare_director_repair_shadow_run` 暗跑对账 legacy tool_results 与 kernel receipts 的文件/source_tool 范围。
