...
多轮确定性修复必须使用 repair kernel scheduler 建模 `priority`、`depends_on`、`round_number`、`max_rounds` 与 cycle breaker；receipt 必须携带 post-check evidence，证明对应 diagnostic 是否消失。迁移旧策略必须先用 `compare_director_repair_shadow_run` 暗跑对账 legacy tool_results 与 kernel receipts 的文件/source_tool 范围。legacy `tool_results` 转 repair_kernel summary 必须走 `ProjectDirectorRepairKernelSummaryV1` + `project_director_repair_kernel_summary`；`build_director_repair_kernel_summary` 是 runtime public 兼容 helper，不得从 `roles.adapters` 调用。
