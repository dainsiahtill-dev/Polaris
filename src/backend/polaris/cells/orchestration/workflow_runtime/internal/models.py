"""Shared data contracts for Workflow orchestration."""
...
def utc_now_iso() -> str:
    ...
    try:
        from .workflow_client import get_workflow_api

        workflow_api = get_workflow_api()
        now_fn = getattr(workflow_api, "now", None)
        if callable(now_fn):
            current = now_fn()
...
class PMWorkflowInput:
    ...
    def from_mapping(cls, raw: Any) -> PMWorkflowInput:
        payload = _normalize_mapping(raw)
        precomputed_raw = payload.get("precomputed_payload")
        precomputed: dict[str, Any] = precomputed_raw if isinstance(precomputed_raw, dict) else {}
...
class DirectorWorkflowInput:
    ...
    def from_mapping(cls, raw: Any) -> DirectorWorkflowInput:
        ...
        metadata_raw = payload.get("metadata")
        metadata: dict[str, Any] = metadata_raw if isinstance(metadata_raw, dict) else {}
        director_config_raw = metadata.get("director_config")
        director_config: dict[str, Any] = director_config_raw if isinstance(director_config_raw, dict) else {}
        ...
        return cls(
            ...
            max_parallel_tasks=_coerce_positive_int(
                raw_max_parallel_tasks,
                3,
            ),
            ready_timeout_seconds=_coerce_positive_int(
                raw_ready_timeout,
                30,
            ),
            task_timeout_seconds=_coerce_positive_int(
                raw_task_timeout,
...
class DirectorTaskInput:
    ...
    def from_mapping(cls, raw: Any) -> DirectorTaskInput:
        ...
        phases: list[str] = (
            [str(item).strip() for item in phases_raw if isinstance(phases_raw, list) and str(item).strip()]
...
class DirectorTaskResult:
    ...
    def from_mapping(cls, raw: Any) -> DirectorTaskResult:
        payload = _normalize_mapping(raw)
        completed_phases_raw = payload.get("completed_phases")
        completed_phases: list[str] = (
            [
                str(item).strip()
                for item in completed_phases_raw
                if isinstance(completed_phases_raw, list) and str(item).strip()
            ]
        ...
        errors: list[str] = (
            [str(item).strip() for item in errors_raw if isinstance(errors_raw, list) and str(item).strip()]
...
class QAWorkflowInput:
    ...
    def from_mapping(cls, raw: Any) -> QAWorkflowInput:
        ...
        raw_task_results: list[Any] = task_results_raw if isinstance(task_results_raw, list) else []
