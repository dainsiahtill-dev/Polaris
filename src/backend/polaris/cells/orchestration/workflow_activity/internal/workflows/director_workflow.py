"""Director workflow for task fan-out and convergence.

Migrated from:
  polaris/cells/orchestration/workflow_runtime/internal/runtime_engine/workflows/director_workflow.py
...
class DirectorWorkflow(WorkflowQueryState):
    ...
    async def run(self, workflow_input: DirectorWorkflowInput) -> DirectorWorkflowResult:
        ...
        task_timeout_seconds = _positive_int(workflow_input.task_timeout_seconds, MAX_WORKFLOW_TIMEOUT_SECONDS)
        self._record_event(
        ...
        while pending:
            ...
            self._record_event(
                ...
                message="Director evaluating ready tasks",
                details={
                    "run_id": workflow_input.run_id,
                ...
                if result is None:
                    ...
                    _record_resident_decision_safe(
                        workflow_input.workspace,
                        {
                            ...
                            "expected_outcome": {"status": "completed", "success": True},
                            "actual_outcome": {"status": "invalid_payload", "success": False},
                            "verdict": "failure",
                            "evidence_refs": ["runtime/results/director.result.json"],
                            "confidence": 0.86,
                        },
