...
def _normalize_llm_event(raw: dict[str, Any], *, source_path: str = "") -> dict[str, Any] | None:
    ...
    terminal = lowered_event in {"llm_call_end", "llm_error", "call_end", "call_error", "invoke_end", "error", "llm_route_terminal"}
...
def collect_llm_events(
    ...
    for base in (*runtime_dirs, workspace / ".polaris" / "runtime", workspace / ".polaris"):
        if base is None:
            continue
        dispatch_log = base / "dispatch" / "log.json"
        ...
        for raw in dispatch_data.get("fail_closed_route_events") or []:
            if not isinstance(raw, dict):
                continue
            item = _normalize_llm_event(raw, source_path=str(dispatch_log))
            if item is not None:
