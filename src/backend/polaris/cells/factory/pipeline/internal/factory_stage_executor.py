...
class OrchestrationStageExecutor:
    ...
    def _resolve_director_binding_fanout(self) -> list[dict[str, str]]:
        """Resolve reachable Director bindings for per-binding dispatch fanout.

        Returns a list of ``{"provider_id": str, "model": str, "binding_id": str}``
        dicts — one per configured AND reachable binding.  Empty list when
        unresolvable or single-binding (fanout not needed).
        """
        try:
    ...
    async def _execute_director_binding_fanout(
        ...
        *,
        ...
        """Execute Director dispatch once per reachable binding in parallel.

        Each binding run carries ``binding_override`` metadata so the orchestration
        worker thread applies the correct thread-local provider override.  Submits
        all bindings, then waits for **every** run to reach a terminal status before
        returning a merged ``CommandResult`` with per-binding metadata.

        Merged status semantics:
        * ``completed`` — every binding run reached ``completed`` or ``success``.
        * ``failed``    — at least one binding run reached a non-success terminal
          status (``failed``, ``cancelled``, ``timeout``, ``partial``).
        """
        ...
        async def _run_binding(binding: dict[str, str]) -> CommandResult:
            ...
            return await service.execute_director_run(
                workspace=workspace,
                tasks=tasks,
        ...
        gathered = await asyncio.gather(
            *[_run_binding(b) for b in bindings],
                ...
                submitted.append(
                    (
                        ...
                        CommandResult(
                            ...
                            status="failed",
                            message=str(item),
                            reason_code="BINDING_FANOUT_ERROR",
                        ),
                    )
                )
            elif isinstance(item, CommandResult):
                submitted.append((bindings[idx], item))
        ...
        for binding, sub_result in submitted:
            ...
            try:
                ...
                logger.warning(
                    "Director binding fanout wait failed for run %s: %s",
                    sub_result.run_id,
                    exc,
                )
        ...
        merged_status = "completed" if fail_count == 0 else "failed"
        ...
        return CommandResult(
            ...
            message=(
                f"Director binding fanout: {len(bindings)} bindings, {success_count} succeeded, {fail_count} failed"
            ),
            metadata=merged_metadata,
        )

    @staticmethod
    def _build_per_binding_route_events(
        per_binding: list[dict[str, Any]],
    ) -> list[dict[str, Any]]:
        """Build terminal route events from per_binding fanout results.

        Each binding (completed, timeout, or failed) gets a synthetic terminal
        route event so that ``llm_route_audit`` can match configured bindings
        to actual dispatch evidence.
        """
    ...
    def _build_fail_closed_director_route_events(
        *,
        ...
        """Generate explicit fail-closed diagnostics for missing director routes.

        When the director dispatch only exercises a subset of configured
        bindings, the llm_route_audit gate fails because it requires evidence
        for every configured binding.  This method inspects the dispatch
        attempts, stage signals, and per_binding terminal route events to
        identify which director routes actually produced evidence, then records
        explicit fail-closed diagnostics for the rest. These diagnostics are
        not successful LLM-call evidence.
        """
        try:
    ...
    def _reclassify_binding_coverage_signals(
        ...
        """Reclassify binding_coverage_incomplete when all bindings have evidence.

        If every configured binding has terminal evidence in per_binding_route_events
        but some timed out, replace the ``director.binding_coverage_incomplete``
        signal with ``director.binding_timeout`` for accurate root-cause attribution.
        """
        ...
        for i, signal in enumerate(stage_signals):
            ...
            stage_signals[i] = {
                ...
                "severity": "error",
                "detail": (
                    f"All director bindings have terminal evidence but "
                    f"{len(timeout_bindings)} timed out: {', '.join(timeout_bindings[:8])}"
                ),
    ...
    def _validate_director_binding_coverage(
        ...
        """Validate all configured director bindings have real LLM terminal evidence.

        Uses ``collect_llm_events`` to gather real LLM events from the workspace
        and ``build_llm_route_audit`` to verify coverage.  Returns
        ``(all_covered, additional_signals)``.  When coverage is incomplete the
        signals contain a single ``director.binding_coverage_incomplete`` error
        with the missing binding keys.

        *additional_events* may contain per-binding terminal route events
        generated from fanout results so the audit can match bindings even
        when JSONL event files are not yet written.
        """
        try:
            from polaris.cells.factory.pipeline.internal.bench_gates import (
            ...
            signals.append(
                {
                    ...
                    "severity": "error",
                    "detail": (
                        f"Not all configured director bindings produced real LLM evidence. "
                        f"Observed={observed_count}, missing={len(missing)}, "
                        f"fail_closed(diagnostic)={fail_closed_count}. "
    ...
    async def _execute_chief_engineer_review(self, run: FactoryRun, context: dict[str, Any]) -> StageResult:
        ...
        for index, task in enumerate(pm_tasks, start=1):
            ...
            try:
                ...
                # Extract provider/model from ce_result metadata (not
                # top-level attrs, which RoleExecutionResultV1 does not have).
                # The kernel propagates profile.provider_id/profile.model into
                # RoleTurnResult.metadata, which flows through
                # _to_contract_result into RoleExecutionResultV1.metadata.
                _ce_meta: dict[str, Any] = dict(ce_result.metadata) if ce_result.metadata else {}
                ce_provider = (
                    str(_ce_meta.get("provider_id") or _ce_meta.get("provider") or "unknown").strip() or "unknown"
                )
                ce_model = str(_ce_meta.get("model") or _ce_meta.get("model_id") or "unknown").strip() or "unknown"
                ce_cache_hit = bool(_ce_meta.get("cache_hit", False))

                # Fail-closed: if provider/model could not be resolved,
                # record the root cause so the CE audit never silently
                # emits "unknown" as if it were a legitimate provider.
                ce_provider_model_unknown = False
                ce_provider_model_unknown_reason = ""
                if ce_provider == "unknown" or ce_model == "unknown":
                    ce_provider_model_unknown = True
                    missing_parts: list[str] = []
                    if ce_provider == "unknown":
                        missing_parts.append("provider_id/provider")
                    if ce_model == "unknown":
                        missing_parts.append("model/model_id")
                    ce_provider_model_unknown_reason = (
                        f"Runtime kernel result metadata did not contain {' and '.join(missing_parts)}; "
                        "check that RoleExecutionKernel propagates profile.provider_id/profile.model "
                        "into RoleTurnResult.metadata"
                    )
                ...
                if not ce_result.ok:
                    ...
                    if ce_provider_model_unknown:
                        error_signal["provider_model_unknown"] = True
                        error_signal["provider_model_unknown_reason"] = ce_provider_model_unknown_reason
                        ...
                        **(
                            {
                                ...
                                "provider_model_unknown_reason": ce_provider_model_unknown_reason,
                            }
                            if ce_provider_model_unknown
                            else {}
                        ),
    ...
    async def _execute_director_dispatch(self, run: FactoryRun, context: dict[str, Any]) -> StageResult:
        ...
        # Re-read error_code/root_cause after reclassification
        for signal in stage_signals:
            if not isinstance(signal, dict):
                continue
            if str(signal.get("severity") or "").strip().lower() != "error":
