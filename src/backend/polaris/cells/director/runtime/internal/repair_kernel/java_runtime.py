...
@dataclass(frozen=True)
class JavaPostPlanning:
    """Internal planning result for conservative Java post repairs."""
    ...
    @property
    def source_tool(self) -> str:
        return self.plan.source_tool if self.plan is not None else JAVA_POST_SOURCE_TOOL


@dataclass(frozen=True)
class JavaPostRun:
    """Internal execution result for conservative Java post repairs."""

    planning: JavaPostPlanning
    ok: bool
    execution_result: RepairExecutionResult | None = None
    plan_decision: PolicyDecision | None = None
    composition_decision: PolicyDecision | None = None
...
def plan_java_post_repair(
    *,
    ...
    """Plan conservative Java post repairs inside the runtime kernel."""

    normalized_base = _normalize_base_files(base_files)
    diagnostics = tuple(normalize_artifact_quality_errors(list(artifact_quality_errors or ())))
    notes = tuple(advisor_notes or ())
    plan = build_java_post_plan(
        base_files=normalized_base,
        diagnostics=diagnostics,
        mode=mode,
    )
    if plan is None:
        return JavaPostPlanning(
            diagnostics=diagnostics,
            plan=None,
            composition=None,
            advisor_notes=notes,
...
def run_java_post_repair(
    *,
    ...
    """Run conservative Java post repair through Plan→Compose→Policy→Execute."""
    ...
    if planning.plan is None:
        return JavaPostRun(
            ...
            ok=False,
            error_code="repair_not_planned",
            error_message="No matching conservative Java post repair plan.",
        )
    if planning.composition is None:
        return JavaPostRun(
            planning=planning,
            ok=False,
            error_code="repair_composition_missing",
            error_message="Java post repair composition was not produced.",
        )
