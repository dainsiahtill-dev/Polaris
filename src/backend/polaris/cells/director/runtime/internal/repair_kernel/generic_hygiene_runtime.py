...
from .generic_hygiene_syntax import PATCH_RESIDUE_CLEANUP_SOURCE_TOOL, build_patch_residue_cleanup_plan
from .policy_gate import PolicyDecision, RepairPolicyContext, RepairPolicyGate


@dataclass(frozen=True)
class PatchResidueCleanupPlanning:
    """Internal planning result for generic patch-residue cleanup."""
    ...
    def source_tool(self) -> str:
        return self.plan.source_tool if self.plan is not None else PATCH_RESIDUE_CLEANUP_SOURCE_TOOL


@dataclass(frozen=True)
class PatchResidueCleanupRun:
    """Internal execution result for generic patch-residue cleanup."""

    planning: PatchResidueCleanupPlanning
    ok: bool
    ...
    *,
    ...
    if planning.plan is None:
        return PatchResidueCleanupRun(
            ...
            error_message="No matching patch-residue cleanup plan.",
        )
    if planning.composition is None:
        return PatchResidueCleanupRun(
            ...
            error_message="Patch-residue cleanup composition was not produced.",
        )
