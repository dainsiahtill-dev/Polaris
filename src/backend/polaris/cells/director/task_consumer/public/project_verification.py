"""Consumer-owned project-verification port; execution owner binds in bootstrap."""
...
def _bind_project_verification_client(port: ProjectVerificationClientPortV1) -> None:
    ...
    with _client_lock:
        if _client is None:
            ...
            raise RuntimeError("director_project_verification_client_conflicting_rebind")
...
def _client_port() -> ProjectVerificationClientPortV1:
    with _client_lock:
        port = _client
    if port is None:
        raise RuntimeError("director_project_verification_client_unbound")
    return port
