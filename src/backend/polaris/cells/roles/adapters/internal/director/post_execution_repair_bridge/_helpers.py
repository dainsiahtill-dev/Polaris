"""Helpers for post-execution repair bridge (projection, debt, collectors)."""
...
def _build_repair_kernel_migration_debt(
    ...
    *,
    ...
    payloads = [_result_payload(item) for item in tool_results]
    # Resolve via package namespace so tests that monkeypatch
    # post_execution_repair_bridge._runtime_executable_source_tools keep working.
    import polaris.cells.roles.adapters.internal.director.post_execution_repair_bridge as _pkg

    executable_source_tools = _pkg._runtime_executable_source_tools()
