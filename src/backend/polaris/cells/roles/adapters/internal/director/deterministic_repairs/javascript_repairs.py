...
def _apply_deterministic_javascript_typescript_annotation_repair(
    adapter: Any,
    *,
    task_id: str,
    artifact_quality_errors: list[str],
) -> list[dict[str, Any]]:
    """Remove TypeScript-only annotations that leaked into plain ``.js`` files."""

    if not any(_looks_like_javascript_typescript_annotation_error(error) for error in artifact_quality_errors):
        return []
    ...
    contract_symbols_by_path: dict[Path, list[str]] = {}
    for rel_path in _javascript_error_file_candidates(artifact_quality_errors, workspace_path=workspace_path):
        ...
        try:
            ...
            continue
        repaired = _strip_typescript_annotations_from_javascript(current)
        repaired, contract_symbols = _repair_javascript_placeholder_export_contracts(
            repaired,
            workspace_path=workspace_path,
            exporter_rel_path=rel_path,
        )
        if repaired == current:
            continue
        updated_by_path[target_path] = repaired
        if contract_symbols:
            contract_symbols_by_path[target_path] = contract_symbols

    return _write_javascript_repair_results(
        adapter,
        workspace_path=workspace_path,
        task_id=task_id,
        updated_by_path=updated_by_path,
        source_tool="deterministic_javascript_typescript_annotation_repair",
        metadata_key="contract_symbols",
        metadata_by_path=contract_symbols_by_path,
    )


def _apply_deterministic_javascript_missing_export_repair(
    adapter: Any,
    *,
    task_id: str,
    artifact_quality_errors: list[str],
) -> list[dict[str, Any]]:
    """Repair JS import/export contract drift with JS syntax only."""

    missing_exports = _parse_javascript_missing_export_errors(artifact_quality_errors)
    workspace_path = Path(str(getattr(adapter, "workspace", "") or "")).resolve()
    if not workspace_path.exists() or not workspace_path.is_dir():
        return []
    missing_exports.extend(_javascript_related_import_contract_targets(workspace_path, missing_exports))
    if any(_looks_like_javascript_export_contract_assertion_error(error) for error in artifact_quality_errors):
        missing_exports.extend(_javascript_export_contract_repair_targets(workspace_path))
    missing_exports = _dedupe_javascript_missing_export_targets(missing_exports)
    if not missing_exports:
        return []

    updated_by_path: dict[Path, str] = {}
    repaired_symbols: dict[Path, list[str]] = {}
    for item in missing_exports:
        symbol = item["symbol"]
        importer_rel = item["importer"]
        ...
        if exporter_path is None:
            continue
        current = updated_by_path.get(exporter_path)
        if current is None:
            try:
                current = exporter_path.read_text(encoding="utf-8")
            except (OSError, UnicodeDecodeError):
                continue

        repaired = _repair_javascript_missing_export(
            module_text=current,
            symbol=symbol,
        ...
        if repaired == current:
            continue
        updated_by_path[exporter_path] = repaired
        repaired_symbols.setdefault(exporter_path, []).append(symbol)
    ...
    return _write_javascript_repair_results(
        ...
        metadata_key="symbols",
        metadata_by_path=repaired_symbols,
    )
