...
class TestTypescriptScaffoldRepair:
    """Tests for generating missing package.json and tsconfig.json."""

    def test_generates_package_json_when_missing(self, tmp_path: Path) -> None:
        """When package.json is missing and error says so, repair creates it."""
        ...
        errors = ["package.json not found in workspace"]

        writes = _apply_deterministic_typescript_scaffold_repair(adapter, task_id="t1", artifact_quality_errors=errors)

        assert len(writes) >= 1
        pkg_write = writes[0]
        assert pkg_write["result"]["file"] == "package.json"
        assert (tmp_path / "package.json").exists()
        content = json.loads((tmp_path / "package.json").read_text(encoding="utf-8"))
        assert "scripts" in content
        assert "build" in content["scripts"]

    def test_package_json_targets_index_entrypoint_even_when_missing(self, tmp_path: Path) -> None:
        """Scaffold should point at dist/index.js so entrypoint repair can generate src/index.ts."""
        from polaris.cells.roles.adapters.internal.director.deterministic_repairs.npm_repairs import (
            _apply_deterministic_typescript_scaffold_repair,
        )
        ...
        src = tmp_path / "src"
        src.mkdir()
        (src / "render.ts").write_text("export const render = () => 'ok';\n", encoding="utf-8")
        adapter = _make_adapter(tmp_path)

        _apply_deterministic_typescript_scaffold_repair(
            adapter,
            task_id="t1",
            artifact_quality_errors=["package.json not found in workspace"],
        )

        content = json.loads((tmp_path / "package.json").read_text(encoding="utf-8"))
        assert content["main"] == "dist/index.js"
        assert content["scripts"]["start"] == "node dist/index.js"

    def test_generates_tsconfig_json_when_missing(self, tmp_path: Path) -> None:
        """When tsconfig.json is missing and error says so, repair creates it."""
        from polaris.cells.roles.adapters.internal.director.deterministic_repairs.npm_repairs import (
            _apply_deterministic_typescript_scaffold_repair,
        )
        ...
        (src / "index.ts").write_text("export const x = 1;\n", encoding="utf-8")
        adapter = _make_adapter(tmp_path)
        errors = ["tsconfig.json missing in workspace root"]

        writes = _apply_deterministic_typescript_scaffold_repair(adapter, task_id="t1", artifact_quality_errors=errors)

        assert len(writes) >= 1
        tsconfig_write = writes[0]
        assert tsconfig_write["result"]["file"] == "tsconfig.json"
        assert (tmp_path / "tsconfig.json").exists()
        content = json.loads((tmp_path / "tsconfig.json").read_text(encoding="utf-8"))
        assert "compilerOptions" in content
        assert "include" in content

    def test_generates_both_when_both_missing(self, tmp_path: Path) -> None:
        """When both package.json and tsconfig.json are missing, repair creates both."""
        from polaris.cells.roles.adapters.internal.director.deterministic_repairs.npm_repairs import (
            _apply_deterministic_typescript_scaffold_repair,
        )
        ...
        (src / "index.ts").write_text("export const x = 1;\n", encoding="utf-8")
        adapter = _make_adapter(tmp_path)
        errors = ["package.json not found", "tsconfig.json missing"]

        writes = _apply_deterministic_typescript_scaffold_repair(adapter, task_id="t1", artifact_quality_errors=errors)

        assert len(writes) == 2
        files_written = {w["result"]["file"] for w in writes}
        assert "package.json" in files_written
        assert "tsconfig.json" in files_written

    def test_noop_when_scaffolding_exists(self, tmp_path: Path) -> None:
        """When scaffolding already exists, repair does nothing."""
        from polaris.cells.roles.adapters.internal.director.deterministic_repairs.npm_repairs import (
            _apply_deterministic_typescript_scaffold_repair,
        )
    ...
    def test_noop_when_no_relevant_errors(self, tmp_path: Path) -> None:
        """When errors don't mention missing scaffolding, repair does nothing."""
        from polaris.cells.roles.adapters.internal.director.deterministic_repairs.npm_repairs import (
            _apply_deterministic_typescript_scaffold_repair,
        )

        adapter = _make_adapter(tmp_path)
        errors = ["TS2339: Property 'x' does not exist"]
