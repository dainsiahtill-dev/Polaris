...
class TestWriteToolPhases:
    ...
    def test_build_prepare_invocation_uses_synthetic_sentinel(self) -> None:
        ...
        assert prepare.shadow_key_hash
        assert isinstance(prepare, SyntheticShadowToolKeyV1)
        assert not hasattr(prepare, "execution_mode")
...
async def test_resolver_adopts_prepare_shadow_for_write_tool(
    ...
    record = await write_registry.start_shadow_task(
        ...
        normalized_args=WriteToolPhases.prepare_shadow_normalized_args({"path": "src/auth.ts", "content": "hello"}),
        spec_key=prepare_inv.shadow_key_hash,
...
async def test_resolver_uses_source_call_identity_not_argument_shape(
    ...
    prepare_inv = WriteToolPhases.build_prepare_invocation(
        ToolInvocation(
            call_id=ToolCallId("call_1"),
            tool_name="write_file",
            arguments={"path": "src/auth.ts", "content": "AAAA"},
            effect_type=ToolEffectType.WRITE,
            execution_mode=ToolExecutionMode.WRITE_SERIAL,
        )
    )
    record = await write_registry.start_shadow_task(
        ...
        candidate_id="prepare_call_1",
        tool_name=WriteToolPhases.prepare_shadow_execution_tool_name(),
        normalized_args=WriteToolPhases.prepare_shadow_normalized_args({"path": "src/auth.ts", "content": "AAAA"}),
        spec_key=prepare_inv.shadow_key_hash,
        env_fingerprint=build_env_fingerprint(),
        policy=_make_policy(),
    )
