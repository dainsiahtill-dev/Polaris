...
def _build_codex_env_from_role_config(state: PmRoleState) -> dict[str, str]:
    from polaris.infrastructure.llm.provider_runtime_adapter import AppLLMRuntimeAdapter

    provider_id, model = load_pm_model_config()
    env: dict[str, str] = {}
    if model:
        env["KERNELONE_CODEX_MODEL"] = model
...
def build_pm_prompt(
    ...
    if is_zh:
        role_boundary_rules = (
            "\n角色权限边界（必遵守）：\n"
            "- User仅通过 UI 指令入口（Discussion/Express）下达意图，不生成直接写文件任务。\n"
            "- Architect任务仅允许读写 `docs/`。\n"
            "- PM 只负责规划与契约编排，不直接执行代码实现。\n"
            "- Chief Engineer负责生成代码施工图与方法级蓝图，不直接提交业务代码。\n"
