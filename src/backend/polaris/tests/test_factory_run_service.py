...
# ── R15-B regression tests for Director fanout route audit / timeout attribution ──


class TestDirectorFanoutRouteAuditR15B:
    """Regression tests for R15-B: Director fanout per_binding terminal route
    evidence and timeout attribution.

    Reproduces the R14 live dispatch scenario: 3 bindings, 2 completed, 1
    timeout.  Missing bindings should be empty, fail_closed diagnostics should
    not hit completed bindings, and stage status may fail but should be
    attributed to timeout (director.binding_timeout), not missing coverage.
    """

    def test_build_per_binding_route_events_generates_terminal_events(self):
        """_build_per_binding_route_events creates terminal route events for
        each per_binding entry with correct provider_id, model, binding_id,
        run_id, and status."""
        per_binding = [
            {
                "provider_id": "openai_compat-1781448928833",
                "model": "qwen3.6-27b-gpu1",
                "binding_id": "director:0:openai_compat-1781448928833:qwen3.6-27b-gpu1",
                "run_id": "director-71b002c31166",
                "status": "completed",
                "message": "Run status: completed",
            },
            {
                "provider_id": "openai_compat-1781448696751",
                "model": "qwen3.6-27b-code-gpu0",
                "binding_id": "director:1:openai_compat-1781448696751:qwen3.6-27b-code-gpu0",
                "run_id": "director-91563602f706",
                "status": "timeout",
                "message": "Run timed out after 600 seconds",
            },
            {
                "provider_id": "openai_compat-1781325474837",
                "model": "qwen3.6-27b-int4",
                "binding_id": "director:2:openai_compat-1781325474837:qwen3.6-27b-int4",
                "run_id": "director-753910650d42",
                "status": "completed",
                "message": "Run status: completed",
            },
        ]
        ...
        assert all(e["invocation"] is True for e in events)
        assert all(e["fail_closed"] is False for e in events)
        assert all(e["cache_hit"] is False for e in events)

        # Check statuses preserved
        statuses = {e["provider_id"]: e["status"] for e in events}
        assert statuses["openai_compat-1781448928833"] == "completed"
        assert statuses["openai_compat-1781448696751"] == "timeout"
        assert statuses["openai_compat-1781325474837"] == "completed"

        # Check binding_ids preserved
        binding_ids = {e["provider_id"]: e["binding_id"] for e in events}
        assert (
            binding_ids["openai_compat-1781448696751"] == "director:1:openai_compat-1781448696751:qwen3.6-27b-code-gpu0"
        )

    def test_build_per_binding_route_events_skips_invalid_entries(self):
        """_build_per_binding_route_events skips entries with missing provider or model."""
        per_binding = [
    ...
    def test_build_fail_closed_excludes_per_binding_evidence(self):
        """_build_fail_closed_director_route_events should NOT generate fail_closed
        diagnostics for bindings that have per_binding_route_events evidence."""
        ...
        with patch(
            ...
            return_value={"director": configured_bindings},
        ):
        ...
        # No fail_closed events — all bindings have evidence
        assert len(events) == 0

    def test_build_fail_closed_only_for_truly_missing_bindings(self):
        """_build_fail_closed_director_route_events generates diagnostics only for
        bindings that lack any evidence (not for completed or timeout bindings)."""
        from unittest.mock import patch
        ...
        # Only p0 and p1 have evidence; p2 is missing
        per_binding_route_events = [
            {
                "event": "llm_route_terminal",
                "role": "director",
        ...
        assert events[0]["model"] == "m2"
        assert events[0]["fail_closed"] is True
        assert events[0]["fail_closed_reason"] == "no_dispatch_evidence_for_binding"

    def test_reclassify_binding_coverage_signals_converts_to_timeout(self):
        """When all bindings have terminal evidence but some timed out,
        _reclassify_binding_coverage_signals replaces
        binding_coverage_incomplete with binding_timeout."""
        from unittest.mock import patch
        ...
        stage_signals = [
            {
                ...
                "severity": "error",
                "detail": "Not all configured director bindings produced real LLM evidence.",
                "missing_bindings": [],
                "observed_count": 10,
                "fail_closed_count": 0,
        ...
        with patch(
            ...
            OrchestrationStageExecutor._reclassify_binding_coverage_signals(
                stage_signals,
                per_binding_route_events,
            )

        assert len(stage_signals) == 1
        assert stage_signals[0]["code"] == "director.binding_timeout"
        assert stage_signals[0]["severity"] == "error"
        assert "d1" in stage_signals[0]["detail"]
        assert stage_signals[0]["timeout_bindings"] == ["director:1:p1:m1"] or "p1" in str(
            stage_signals[0]["timeout_bindings"]
        )

    def test_reclassify_noop_when_not_all_bindings_have_evidence(self):
        """_reclassify_binding_coverage_signals does NOT reclassify when not all
        configured bindings have terminal evidence."""
        from unittest.mock import patch
        ...
        stage_signals = [
            {
                "code": "director.binding_coverage_incomplete",
                "severity": "error",
                "detail": "Missing bindings: p2|m2",
                "missing_bindings": ["p2|m2|d2"],
            }
        ]
        ...
        # Should remain binding_coverage_incomplete
        assert stage_signals[0]["code"] == "director.binding_coverage_incomplete"

    def test_reclassify_noop_when_no_timeout(self):
        """_reclassify_binding_coverage_signals does NOT reclassify when all
        bindings completed without timeout."""
        from unittest.mock import patch
    ...
    def test_r14_live_dispatch_scenario_missing_bindings_empty(self):
        """Reproduces R14 live dispatch: 3 bindings, 2 completed, 1 timeout.
        missing_bindings from build_llm_route_audit should be empty when all
        bindings have per_binding_route_events evidence."""
        ...
        per_binding_route_events = OrchestrationStageExecutor._build_per_binding_route_events(per_binding)

        expected = {"director": configured_bindings}
        ...
        # All bindings have evidence — missing should be empty
        assert missing == [], f"Expected empty missing_bindings, got: {missing}"
        assert director_result.get("observed_count", 0) == 3

    def test_r14_fail_closed_not_generated_for_completed_bindings(self):
        """In R14 scenario, fail_closed diagnostics should NOT be generated for
        the 2 completed bindings."""
        from unittest.mock import patch
        ...
        # All 3 bindings have evidence — 0 fail_closed events
        assert len(events) == 0
        fail_closed_providers = {e.get("provider_id") for e in events}
        assert "openai_compat-1781448928833" not in fail_closed_providers
        assert "openai_compat-1781325474837" not in fail_closed_providers

    def test_r14_stage_root_cause_is_timeout_not_missing(self):
        """When all bindings have terminal evidence but one timed out, the stage
        root cause should be director.binding_timeout, not
        director.binding_coverage_incomplete."""
        from unittest.mock import patch
        ...
        stage_signals = [
            {
                "code": "director.binding_coverage_incomplete",
                "severity": "error",
                "detail": "Not all configured director bindings produced real LLM evidence. Missing: p1|m1",
                "missing_bindings": [],
                "observed_count": 3,
                "fail_closed_count": 0,
                "multi_route_required": True,
