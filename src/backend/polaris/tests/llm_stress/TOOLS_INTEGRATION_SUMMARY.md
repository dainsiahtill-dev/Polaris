...
|-----|-------|---------|---------|---------|
| PM | PM | 21 | 只读 + 任务管理 | 不写代码，专注规划 |
| Architect | Architect | **11** | 纯只读 | 仅架构分析，无执行权限 |
| ChiefEngineer | ChiefEngineer | 21 | 只读 + 执行 | 技术分析，不写代码 |
| Director | Director | **26** | 全权限 | 唯一代码编辑权限 |
| QA | QA | 21 | 只读 + 测试 | 审查但不修改 |
