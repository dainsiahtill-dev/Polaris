...
def _production_python_files() -> list[Path]:
    return [
        ...
        if "__pycache__" not in path.parts and "tests" not in path.parts and not path.name.startswith("test_")
    ]
...
def test_production_code_imports_canonical_terminal_package() -> None:
    """Production code must not route through the terminal_console shim."""
    violations: list[str] = []
