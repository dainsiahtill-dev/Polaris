...
from polaris.kernelone.llm.toolkit import protocol, protocol_kernel
...
_FILES = (
    ...
    _BACKEND_ROOT / "polaris" / "kernelone" / "llm" / "toolkit" / "protocol_kernel.py",
...
def test_protocol_uses_explicit_file_operation_validation_result_name() -> None:
    ...
    assert hasattr(protocol_kernel, "FileOpValidationResult")
    ...
    assert not hasattr(protocol_kernel, "ValidationResult")
