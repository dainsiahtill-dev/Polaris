...
def test_toolkit_does_not_export_canonical_tool_call_alias() -> None:
    """Parser APIs should expose ToolCall directly, not retired aliases."""
    ...
    assert not hasattr(canonical, "ParsedToolCall")
    assert not hasattr(parsers, "CanonicalToolCall")
    assert not hasattr(parsers, "ParsedToolCall")
    assert not hasattr(toolkit, "CanonicalToolCall")
    assert not hasattr(toolkit, "ParsedToolCall")
    assert "CanonicalToolCall" not in parsers.__all__
    assert "ParsedToolCall" not in parsers.__all__
    assert "CanonicalToolCall" not in toolkit.__all__
    assert "ParsedToolCall" not in toolkit.__all__
...
def test_toolkit_sources_do_not_reintroduce_tool_call_aliases() -> None:
    """Block retired parser aliases for the canonical ToolCall type."""
    offenders: list[str] = []
    for path in sorted((PARSERS_ROOT, TOOLKIT_ROOT)):
        for source_file in sorted(path.glob("*.py")):
            source = source_file.read_text(encoding="utf-8")
            if (
                "CanonicalToolCall = ToolCall" in source
                or "ParsedToolCall" in source
                or '"CanonicalToolCall"' in source
                or '"ParsedToolCall"' in source
            ):
