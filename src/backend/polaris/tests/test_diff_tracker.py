...
class TestCaptureGitBaselineFailure:
    def test_returns_empty_dict_on_exception(self, tmp_path: Path, caplog):
        ...
        with (
            patch.object(tracker._cmd_svc, "run", side_effect=RuntimeError("git broken")),
    ...
    def test_logs_with_exc_info(self, tmp_path: Path, caplog):
        ...
        with (
            patch.object(tracker._cmd_svc, "run", side_effect=OSError("permission denied")),
            caplog.at_level(logging.WARNING),
        ):
...
class TestFilesystemBaselineStatFailure:
    ...
    def test_walk_failure_yields_warning_and_empty(self, tmp_path: Path, caplog):
        ...
        with (
            patch(
                "polaris.cells.workspace.integrity.internal.diff_tracker.os.walk", side_effect=OSError("disk exploded")
            ),
            caplog.at_level(logging.WARNING),
        ):
            baseline = tracker._capture_filesystem_baseline()
...
class TestGetGitChangesFailure:
    def test_returns_empty_snapshot_on_exception(self, tmp_path: Path, caplog):
        ...
        with (
            patch.object(tracker._cmd_svc, "run", side_effect=RuntimeError("network gone")),
            caplog.at_level(logging.WARNING),
        ):
    ...
    def test_logs_with_exc_info(self, tmp_path: Path, caplog):
        ...
        with (
            patch.object(tracker._cmd_svc, "run", side_effect=ValueError("bad returncode")),
            caplog.at_level(logging.WARNING),
        ):
...
class TestHelperSetFetchFailures:
    ...
    def test_returns_empty_set_and_warns(self, tmp_path: Path, caplog, method_name: str, log_fragment: str):
        tracker = _make_tracker(tmp_path)

        with patch.object(tracker._cmd_svc, "run", side_effect=RuntimeError("oops")), caplog.at_level(logging.WARNING):
            result = getattr(tracker, method_name)()

        assert result == set(), f"{method_name} must return empty set on failure"
        assert any(log_fragment in r.message for r in caplog.records), f"Warning containing '{log_fragment}' expected"
