...
class TestToCompatProjection:
    """Tests for to_compat_projection method."""

    def test_pm_subprocess_projection(self) -> None:
        """Project to pm_subprocess compatibility format."""
        event = CanonicalLogEventV2(
            message="subprocess output",
            raw={"text": "raw text"},
        )
        projection = event.to_compat_projection("pm_subprocess")

        assert projection["type"] == "line"
        assert projection["text"] == "raw text"

    def test_director_console_projection(self) -> None:
        """Project to director_console compatibility format."""
        event = CanonicalLogEventV2(message="console output")
        projection = event.to_compat_projection("director_console")

        assert projection["type"] == "line"

    def test_pm_llm_projection(self) -> None:
        """Project to pm_llm compatibility format."""
        event = CanonicalLogEventV2(
            actor="assistant",
            kind="output",
            raw={"data": "llm data"},
        )
        projection = event.to_compat_projection("pm_llm")
    ...
    def test_runtime_events_projection(self) -> None:
        """Project to runtime_events compatibility format."""
        event = CanonicalLogEventV2(
            kind="state",
            actor="runtime",
            message="state changed",
        )
        projection = event.to_compat_projection("runtime_events")
    ...
    def test_pm_log_projection(self) -> None:
        """Project to pm_log compatibility format."""
        event = CanonicalLogEventV2(
            kind="action",
            compat_output={"result": "ok"},
            compat_input={"command": "test"},
        )
        projection = event.to_compat_projection("pm_log")

        assert projection["type"] == "action"
        assert projection["output"] == {"result": "ok"}
        assert projection["input"] == {"command": "test"}

    def test_unknown_compat_channel(self) -> None:
        """Unknown compatibility channel produces minimal projection."""
        event = CanonicalLogEventV2(message="test")
        projection = event.to_compat_projection("unknown_channel")
...
class TestCompatChannelMapping:
    """Tests for COMPAT_CHANNEL_MAPPING."""

    def test_contains_expected_channels(self) -> None:
        """Mapping contains all expected compatibility channels."""
        expected = {
            "pm_subprocess",
            "director_console",
            "runlog",
...
class TestNormalizeCompatEvent:
    """Tests for normalize_compat_event function."""

    def test_normalize_pm_subprocess(self) -> None:
        """Normalize pm_subprocess event."""
        raw = {"text": "output line", "seq": 1}
        event = normalize_compat_event(raw, "pm_subprocess", run_id="run-1")
    ...
    def test_normalize_pm_llm(self) -> None:
        ...
        event = normalize_compat_event(raw, "pm_llm")

        assert event.channel == "llm"
        assert event.actor == "PM"  # from COMPAT_CHANNEL_MAPPING, raw has no "actor"
    ...
    def test_normalize_runtime_events(self) -> None:
        ...
        event = normalize_compat_event(raw, "runtime_events")
    ...
    def test_normalize_unknown_channel_defaults(self) -> None:
        ...
        event = normalize_compat_event(raw, "unknown_channel")
    ...
    def test_normalize_extracts_message_from_various_fields(self) -> None:
        ...
        event = normalize_compat_event(raw, "pm_log")
    ...
    def test_normalize_preserves_compat_fields(self) -> None:
        """Compatibility fields are populated."""
        raw = {"name": "compat_name", "output": {"result": "ok"}, "input": {"cmd": "test"}}
        event = normalize_compat_event(raw, "pm_log")

        assert event.compat_name == "compat_name"
        assert event.compat_output == {"result": "ok"}
        assert event.compat_input == {"cmd": "test"}

    def test_normalize_empty_raw(self) -> None:
        """Empty raw dict handled gracefully."""
        event = normalize_compat_event({}, "pm_log")
    ...
    def test_normalize_with_run_id(self) -> None:
        ...
        event = normalize_compat_event({}, "pm_log", run_id="run-abc")
