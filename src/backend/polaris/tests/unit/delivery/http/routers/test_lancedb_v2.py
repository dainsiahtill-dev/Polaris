"""Tests for Polaris v2 LanceDB router.

Covers GET /v2/lancedb/status.
External services are mocked to avoid LanceDB dependencies.
"""
...
# ---------------------------------------------------------------------------
# GET /v2/lancedb/status
# ---------------------------------------------------------------------------


@pytest.mark.asyncio
async def test_v2_lancedb_status_success(client: AsyncClient) -> None:
    """GET /v2/lancedb/status should return LanceDB status."""
    with patch(
        "polaris.delivery.http.routers.lancedb.get_lancedb_status",
        return_value={"available": True, "tables": 3},
    ) as mock_status:
        response = await client.get("/v2/lancedb/status")
...
@pytest.mark.asyncio
async def test_v2_lancedb_status_unavailable(client: AsyncClient) -> None:
    """GET /v2/lancedb/status should handle unavailable LanceDB."""
    with patch(
        "polaris.delivery.http.routers.lancedb.get_lancedb_status",
        return_value={"available": False, "error": "not installed"},
    ) as mock_status:
        response = await client.get("/v2/lancedb/status")
        assert response.status_code == 200
        data = response.json()
        assert data["available"] is False
        assert data["error"] == "not installed"
        mock_status.assert_called_once()
