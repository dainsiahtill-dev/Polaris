...
class TestFileLockAdapter:
    ...
    def test_acquire_and_release(self, tmp_path: Path) -> None:
        ...
        release_result = asyncio.get_event_loop().run_until_complete(adapter.release("resource-1", "holder-1"))
    ...
    def test_acquire_timeout(self, tmp_path: Path) -> None:
        ...
        asyncio.get_event_loop().run_until_complete(adapter1.release("res", "h1"))
    ...
    def test_release_wrong_holder(self, tmp_path: Path) -> None:
        ...
        result = asyncio.get_event_loop().run_until_complete(adapter.release("res", "h2"))
    ...
    def test_extend(self, tmp_path: Path) -> None:
        ...
        extended = asyncio.get_event_loop().run_until_complete(adapter.extend("res", "h1", additional_seconds=10.0))
    ...
    def test_extend_wrong_holder(self, tmp_path: Path) -> None:
        ...
        extended = asyncio.get_event_loop().run_until_complete(adapter.extend("res", "h2", additional_seconds=10.0))
        assert extended is False

    def test_is_held(self, tmp_path: Path) -> None:
        adapter = FileLockAdapter(str(tmp_path))
        held_before = asyncio.get_event_loop().run_until_complete(adapter.is_held("res"))
        assert held_before is False

        asyncio.get_event_loop().run_until_complete(
            adapter.acquire("res", "h1", LockOptions(timeout_seconds=1.0, ttl_seconds=10.0))
        )
        held_after = asyncio.get_event_loop().run_until_complete(adapter.is_held("res"))
        assert held_after is True

        asyncio.get_event_loop().run_until_complete(adapter.release("res", "h1"))
        held_released = asyncio.get_event_loop().run_until_complete(adapter.is_held("res"))
    ...
    def test_is_held_expired(self, tmp_path: Path) -> None:
        ...
        time.sleep(0.05)
        held = asyncio.get_event_loop().run_until_complete(adapter.is_held("res"))
