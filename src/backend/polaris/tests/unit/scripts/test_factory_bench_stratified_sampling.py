...
class TestImmutableAuditPackageGeneration:
    ...
    @pytest.mark.parametrize("lang", ["typescript", "javascript", "python", "go", "rust", "cpp", "java"])
    def test_language_sample_produces_audit_package(
        ...
        lang_projects = [p for p in projects if str(p.get("primary_language") or "").strip().lower() == lang]
    ...
    def test_stratified_sample_covers_all_levels(self, sample_projects: list[dict[str, Any]]) -> None:
        """Stratified sample covers L1-L12."""
        levels = {int(p.get("level") or 0) for p in sample_projects}
        expected_levels = set(range(1, 13))
        assert expected_levels.issubset(levels), f"Missing levels: {expected_levels - levels}"

    def test_stratified_sample_covers_all_languages(self, language_sample_projects: list[dict[str, Any]]) -> None:
        """Stratified sample covers all 7 languages."""
        langs = {str(p.get("primary_language") or "").strip().lower() for p in language_sample_projects}
        expected_langs = {"typescript", "javascript", "python", "go", "rust", "cpp", "java"}
        assert expected_langs.issubset(langs), f"Missing languages: {expected_langs - langs}"

    def test_full_stratified_audit_packages(self, sample_projects: list[dict[str, Any]], tmp_path: Path) -> None:
        ...
        for project in sample_projects:
            ...
            assert ".2." in result_path.name


class TestRequirementsDocGeneration:
    """Verify requirements doc generation for stratified samples."""

    def test_requirements_doc_contains_language_contract(self, sample_projects: list[dict[str, Any]]) -> None:
    ...
    def test_requirements_doc_contains_feature_keywords(self, projects: list[dict[str, Any]]) -> None:
    ...
    def test_requirements_doc_contains_checks(self, sample_projects: list[dict[str, Any]]) -> None:
...
class TestProjectCatalogIntegrity:
    ...
    def test_all_projects_have_valid_language(self, projects: list[dict[str, Any]]) -> None:
        ...
        for project in projects:
            lang = str(project.get("primary_language") or "").strip().lower()
            assert lang in valid_languages, f"Project {project.get('id')} has invalid language: {lang}"
    ...
    def test_level_distribution(self, projects: list[dict[str, Any]]) -> None:
        ...
        for level in range(1, 13):
            assert level_counts[level] == 10, f"Level L{level} has {level_counts[level]} projects, expected 10"

    def test_language_distribution(self, projects: list[dict[str, Any]]) -> None:
        """Each language has at least 10 projects."""
        from collections import Counter

        lang_counts = Counter(str(p.get("primary_language") or "").strip().lower() for p in projects)
...
class TestDryRunAndLimit:
    ...
    def test_dry_run_flag_exists(self) -> None:
        ...
        # Verify argparse accepts --dry-run by parsing test args
        with patch("sys.argv", ["run_factory_bench", "--dry-run", "--limit", "1", "--work-dir", "/tmp/test"]):
            # If argparse rejects --dry-run, this will raise SystemExit
    ...
    def test_limit_flag_exists(self) -> None:
        ...
        # Verify argparse accepts --limit by parsing test args
        ap = argparse.ArgumentParser()
        ap.add_argument("--limit", type=int, default=0)
        args = ap.parse_args(["--limit", "5"])
        assert args.limit == 5
    ...
    def test_levels_filter(self, projects: list[dict[str, Any]]) -> None:
        ...
        test_levels = [1, 5, 12]
        filtered = [p for p in projects if int(p.get("level") or 0) in test_levels]
...
class TestDryRunEndToEnd:
    ...
    def test_dry_run_generates_audit_packages_for_all_levels(
        self, projects: list[dict[str, Any]], tmp_path: Path
    ) -> None:
        """Dry-run generates audit packages covering L1-L12."""
        audit_dir = tmp_path / "audits" / "dry-run"
        audit_dir.mkdir(parents=True, exist_ok=True)

        catalog_hash = "test_hash"
        generated_levels: set[int] = set()
        generated_langs: set[str] = set()

        # Simulate dry-run logic from main()
        for project in projects:
            pid = str(project.get("id") or "")
            level = int(project.get("level") or 0)
            lang = str(project.get("primary_language") or "")
            ...
            generated_levels.add(level)
            generated_langs.add(lang)

        # Verify all levels L1-L12 are covered
        expected_levels = set(range(1, 13))
        assert expected_levels.issubset(generated_levels), f"Missing levels: {expected_levels - generated_levels}"

        # Verify all 7 languages are covered
        expected_langs = {"typescript", "javascript", "python", "go", "rust", "cpp", "java"}
        assert expected_langs.issubset(generated_langs), f"Missing languages: {expected_langs - generated_langs}"

    def test_dry_run_with_limit_restricts_output(self, projects: list[dict[str, Any]], tmp_path: Path) -> None:
        """Dry-run with --limit restricts number of audit packages."""
        limit = 12  # One per level
        limited = projects[:limit]
        audit_dir = tmp_path / "audits" / "dry-run"
        audit_dir.mkdir(parents=True, exist_ok=True)
        ...
        for project in limited:
            ...
            project_audit = {
                ...
                "level": level,
                "primary_language": lang,
                "record": {"project_id": pid, "level": level, "primary_language": lang, "dry_run": True},
                "completed_at": "2026-01-01T00:00:00Z",
            }
            result_path = _write_immutable_json(audit_file, project_audit)
            assert result_path.exists()
            generated.append(json.loads(result_path.read_text(encoding="utf-8")))

        # Verify limit is respected
        assert len(generated) == limit

        # Verify each audit package has correct structure
        for audit in generated:
            assert audit["catalog_schema_version"] == "factory-bench/2"
            assert audit["run_id"] == "dry-run"
            assert audit["record"]["dry_run"] is True

    def test_dry_run_audit_packages_have_immutable_paths(self, projects: list[dict[str, Any]], tmp_path: Path) -> None:
        """Dry-run audit packages use immutable path resolution."""
        audit_dir = tmp_path / "audits" / "dry-run"
        audit_dir.mkdir(parents=True, exist_ok=True)

        # Generate audit for first project twice
        project = projects[0]
        pid = str(project.get("id") or "")
        audit_file = audit_dir / f"{pid}.audit.json"

        first_payload = {"run": 1, "project_id": pid}
        second_payload = {"run": 2, "project_id": pid}

        first_path = _write_immutable_json(audit_file, first_payload)
        second_path = _write_immutable_json(audit_file, second_payload)
    ...
    def test_dry_run_covers_each_level_with_distinct_language(
        self, projects: list[dict[str, Any]], tmp_path: Path
    ) -> None:
        """Dry-run can cover each level with a distinct language sample."""
        audit_dir = tmp_path / "audits" / "dry-run"
        audit_dir.mkdir(parents=True, exist_ok=True)

        # Sample one project per level, tracking languages
        seen_levels: set[int] = set()
        level_lang_map: dict[int, str] = {}
        ...
        # Verify all levels covered
        assert seen_levels == set(range(1, 13))
        # Verify each level has a language assigned
        for level in range(1, 13):
            assert level in level_lang_map
            assert level_lang_map[level] in {"typescript", "javascript", "python", "go", "rust", "cpp", "java"}


class TestAuditPackageImmutability:
    """Verify audit packages are truly immutable."""

    def test_writing_same_project_twice_creates_two_files(self, tmp_path: Path) -> None:
