...
This module provides an early-initialization bridge that maps deprecated
``POLARIS_*`` environment variables to their canonical ``KERNELONE_*``
counterparts. It must be invoked before any ``polaris.kernelone`` import occurs,
so that the kernel layer always sees the canonical prefix.
...
- ``POLARIS_*`` is accepted at the user-facing boundary during the env migration.
...
_POLARIS_TO_KERNELONE: dict[str, str] = {
    ...
    "POLARIS_AUTHORIZED_SCOPE_PATHS": "KERNELONE_AUTHORIZED_SCOPE_PATHS",
    "POLARIS_DIRECTOR_DISPATCH_TIMEOUT_SETTLE_GRACE_SECONDS": (
        "KERNELONE_DIRECTOR_DISPATCH_TIMEOUT_SETTLE_GRACE_SECONDS"
    ),
    "POLARIS_FACTORY_BENCH_INTERNAL_ENABLED": "KERNELONE_FACTORY_BENCH_INTERNAL_ENABLED",
    "POLARIS_FACTORY_DIRECTOR_FIRST_MATERIALIZATION_MIN_BUDGET_SECONDS": (
        "KERNELONE_FACTORY_DIRECTOR_FIRST_MATERIALIZATION_MIN_BUDGET_SECONDS"
    ),
    "POLARIS_FACTORY_JETSTREAM_FANOUT_TIMEOUT_SECONDS": "KERNELONE_FACTORY_JETSTREAM_FANOUT_TIMEOUT_SECONDS",
    "POLARIS_FACTORY_LIVE_LLM_PREFLIGHT": "KERNELONE_FACTORY_LIVE_LLM_PREFLIGHT",
    "POLARIS_FACTORY_QA_REQUIRE_LLM_JUDGEMENT": "KERNELONE_FACTORY_QA_REQUIRE_LLM_JUDGEMENT",
    "POLARIS_FACTORY_QUALITY_GATE_RESERVED_BUDGET_SECONDS": ("KERNELONE_FACTORY_QUALITY_GATE_RESERVED_BUDGET_SECONDS"),
    "POLARIS_FACTORY_WORKSPACE_VALIDATION": "KERNELONE_FACTORY_WORKSPACE_VALIDATION",
    "POLARIS_FACTORY_WORKSPACE_VALIDATION_DELIVERY_DEPTH_CONTRACT": (
        "KERNELONE_FACTORY_WORKSPACE_VALIDATION_DELIVERY_DEPTH_CONTRACT"
    ),
    "POLARIS_FACTORY_WORKSPACE_VALIDATION_ENTRYPOINT_SMOKE": (
        "KERNELONE_FACTORY_WORKSPACE_VALIDATION_ENTRYPOINT_SMOKE"
    ),
    "POLARIS_FACTORY_WORKSPACE_VALIDATION_INSTALL_DEPENDENCIES": (
        "KERNELONE_FACTORY_WORKSPACE_VALIDATION_INSTALL_DEPENDENCIES"
    ),
    "POLARIS_INSTANCE_HOME": "KERNELONE_INSTANCE_HOME",
    "POLARIS_INSTANCE_ID": "KERNELONE_INSTANCE_ID",
    "POLARIS_INSTANCE_KIND": "KERNELONE_INSTANCE_KIND",
    "POLARIS_INSTANCE_WATCHDOG_ENABLED": "KERNELONE_INSTANCE_WATCHDOG_ENABLED",
    "POLARIS_INSTANCE_WATCHDOG_INTERVAL_SECONDS": "KERNELONE_INSTANCE_WATCHDOG_INTERVAL_SECONDS",
    "POLARIS_INTERNAL_BENCH_ENABLED": "KERNELONE_INTERNAL_BENCH_ENABLED",
    "POLARIS_LLM_STATUS_RUNTIME_SKIP_TTL_SECONDS": "KERNELONE_LLM_STATUS_RUNTIME_SKIP_TTL_SECONDS",
    "POLARIS_PM_DETERMINISTIC_CONTRACTS": "KERNELONE_PM_DETERMINISTIC_CONTRACTS",
}


def normalize_env_prefix() -> None:
    """Map deprecated ``POLARIS_*`` env vars to ``KERNELONE_*`` before kernel import."""
    mapped: list[str] = []
    ...
    if mapped:
        ...
        message = (
            "[ENV PREFIX] The following POLARIS_* environment variables are deprecated "
            "and will be removed in a future release. Please migrate to KERNELONE_*:\n"
        )
        for key in mapped:
            message += f"  {key}  ->  {_POLARIS_TO_KERNELONE[key]}\n"
        message += "See: https://github.com/anthropics/polaris/blob/main/docs/env-migration.md\n"
