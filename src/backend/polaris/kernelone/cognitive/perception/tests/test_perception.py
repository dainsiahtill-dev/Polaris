...
async def test_semantic_parser_recognizes_create(perception):
    graph, uncertainty = await perception.process("Create a new API endpoint for user authentication")
...
async def test_semantic_parser_recognizes_modify(perception):
    graph, uncertainty = await perception.process("Update the user authentication module")
    assert graph.nodes[0].intent_type == "modify_file"


@pytest.mark.asyncio
async def test_semantic_parser_recognizes_read(perception):
    graph, uncertainty = await perception.process("Read the file at src/main.py")
    assert graph.nodes[0].intent_type == "read_file"


@pytest.mark.asyncio
async def test_semantic_parser_recognizes_delete(perception):
    graph, uncertainty = await perception.process("Delete the temporary files")
...
async def test_uncertainty_drives_path_selection(perception):
    ...
    graph_high, uncertainty_high = await perception.process("Read the file at src/main.py")
    assert uncertainty_high.recommended_action in ("bypass", "fast_think")

    # Ambiguous message -> full pipe
    graph_low, uncertainty_low = await perception.process("Help me with something maybe")
...
async def test_unstated_needs_detection(perception):
    graph, uncertainty = await perception.process("Create a new test file")
...
async def test_intent_graph_contains_surface_intent(perception):
    graph, uncertainty = await perception.process("Fix the bug in login")
