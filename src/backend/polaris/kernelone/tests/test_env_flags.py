...
def test_invalid_value_fails_closed_without_echoing_raw_value() -> None:
    decision = resolve_env_flag(
        ("FEATURE_A",),
        environ={"FEATURE_A": "secret-looking-invalid-value"},
    )

    assert decision.enabled is False
    assert decision.reason == "invalid_value"
    assert decision.configured_names == ("FEATURE_A",)
    assert "secret-looking-invalid-value" not in repr(decision)
...
def test_duplicate_alias_names_are_deduplicated() -> None:
    decision = resolve_env_flag(
        ("FEATURE_A", " FEATURE_A ", "FEATURE_A"),
        environ={"FEATURE_A": "1"},
    )
