"""Strict, auditable, fail-closed environment feature-flag resolution."""
...
class EnvFlagDecision:
    ...
    reason: EnvFlagReason


def _normalized_unique_names(names: Sequence[str]) -> tuple[str, ...]:
    normalized: list[str] = []
    seen: set[str] = set()
...
def resolve_env_flag(
    ...
    *,
    ...
    normalized_names = _normalized_unique_names(names)
    if not normalized_names:
        raise ValueError("at least one environment variable name is required")

    source: Mapping[str, str] = os.environ if environ is None else environ
    configured = tuple(
        (name, source[name]) for name in normalized_names if name in source
    )
