...
_BUILTIN_REGISTRY: dict[str, dict[str, Any]] = {
    ...
    "edit_blocks": {
        "category": "write",
        "description": "Apply an edit to a file. TWO accepted forms:\n\n(1) SEARCH/REPLACE block(s) (use when you can copy the exact original):\n<<<< SEARCH[:filepath]\n<original code copied verbatim>\n====\n<new code>\n>>>> REPLACE\n\n(2) LINE-RANGE form (easiest — no need to reproduce the original text): pass `file`, `start` and `end` (1-based, inclusive line numbers, e.g. from repo_read_slice) and `replace` = the full new source for those lines. The tool reads the exact current lines itself, so there is no SEARCH-match risk.\n\nBenefits: zero JSON escaping issues, native code formatting preserved, character-level hallucination tolerant ('return0' -> 'return 0'), multi-file edits in one call.",
        ...
        "arguments": [
            ...
            {"name": "start", "type": "integer", "required": False},
            {"name": "end", "type": "integer", "required": False},
            {"name": "replace", "type": "string", "required": False},
        ],
        "response_format_hint": "Edit confirmation with files modified and blocks applied",
        "required_any": [["blocks", "start"]],
        "required_doc": "Either args.blocks (SEARCH/REPLACE) OR args.file+start+end+replace (line-range). args.file optional if specified in blocks.",
    },
    "repo_apply_diff": {
        "category": "write",
