...
This is a KernelOne-level shared repository: it has zero polaris-cell imports
(stdlib + KernelOne storage/fs only). History-root resolution is delegated to
``resolve_storage_roots`` which honours the business-layer roots resolver
registered at bootstrap, so the workspace-anchored ``<workspace>/<metadata_dir>/
history`` layout is preserved identically to the previous cell-local resolver.
"""
