...
| RB-09 | P1 | Closed | `edit_file` line-range calls could drop weak-model `new_text` body aliases after schema normalization mapped them to `replace`. | Full toolkit normalization suite exposed `TestEditFileSynonyms::test_line_range_body_synonym_maps_to_content[new_text]`: `new_text` stayed as `replace`, then handler-facing line-range content was missing. | `edit_file` line-range normalization treats schema-normalized `replace` as the line replacement body when `search` mode is not active, preserving weak-model body aliases without weakening search/replace semantics. | `rtk pytest src/backend/polaris/kernelone/llm/toolkit/tests/test_tools_normalization.py -q`; `rtk ruff check src/backend/polaris/kernelone/llm/toolkit/tool_normalization/normalizers/_edit_file.py`; `rtk mypy src/backend/polaris/kernelone/llm/toolkit/tool_normalization/normalizers/_edit_file.py`; `rtk proxy python -m py_compile src/backend/polaris/kernelone/llm/toolkit/tool_normalization/normalizers/_edit_file.py`. |
