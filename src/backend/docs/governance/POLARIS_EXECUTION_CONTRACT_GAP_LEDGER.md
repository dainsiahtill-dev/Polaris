...
| H-14 | P1 | QA failure taxonomy is duplicated across QA cell, Director repair, workflow QA, and integration QA strings. | `qa.audit_verdict.public` now exposes `build_qa_failure_classification_v1`; QA verdict engine and Director repair construct classifications through it, and integration QA emits a canonical `qa_failure_classification` beside its legacy report field. Workflow QA/runtime UI strings still need migration to the shared builder or canonical projection constants. | Partial | Public contract, verdict engine, Director repair, integration QA, ruff, and py_compile targeted tests pass. Close after workflow QA/runtime projection no longer hard-code local failure taxonomy strings. |
