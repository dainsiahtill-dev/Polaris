...
| H-16 | P0 | Final provider request evidence still has multiple observable shapes. | Top-level LLM event projection, realtime log refs, Factory bench collection, and CE evidence extraction now normalize `context_snapshot_ref` to a canonical 24-hex ContextStore hash and reject non-replayable refs. Structured LLM call snapshot parity is still unproven. | Partial | Event projection, realtime refs, Factory bench collector, and CE mirror tests pass. Close after `call_structured()` proves it writes a readable final-request ContextStore snapshot with the same canonical ref shape. |
