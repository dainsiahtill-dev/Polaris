...
| H-10 | P0 | Streaming provider responses can still decode native tool calls without the same `tool_dispatch_dropped` fail-closed lifecycle used by non-streaming decisions. | Non-streaming path has lifecycle receipts; streaming path still needs equivalent native-count/no-dispatch guard and non-empty provider response hash. | Open | Add a streaming invalid-native-tool-call regression proving Run Ledger `tool_lifecycle.dropped_count=1` and QA/runtime projection cannot report success. |
| H-11 | P0 | `TaskBoundaryVerdict` exists but is not appended by the normal Director completion/failure path. | Bench runner calls `evaluate_task_boundary_verdict`, while production Director/RoleExecution paths still mostly emit deferred follow-up or generic no-materialized-changes evidence. | Open | Add Director materialization and manifest-entrypoint boundary tests for `INCOMPLETE_MATERIALIZATION`, `MISSING_ENTRYPOINT_TARGET`, and `TOOL_DISPATCH_DROPPED`. |
| H-12 | P0 | Director status and runtime snapshot still have local/workflow fallback paths that can diverge from Run Ledger terminal evidence. | Runtime projection overlays Run Ledger failures, but `/v2/director/status` defaults to `source=local`, and snapshots can still backfill `pm.state.json` / `director.result.json`. | Open | Add status projection test proving Run Ledger `TOOL_DISPATCH_DROPPED` overrides local `IDLE`; make UI/API default to projection/auto source. |
| H-13 | P0 | QA verdict engine is ledger-aware but not yet the default authoritative route. | `QAVerdictEngine` can build typed envelopes, but Task Market QA defaults to `shadow`; public QA audit still returns old `QaAuditResultV1` without envelope refs. | Open | Switch default to authoritative only after QA public envelope builder and mismatch tests pass; prove legacy PASS cannot override ledger BLOCKED. |
| H-14 | P1 | QA failure taxonomy is duplicated across QA cell, Director repair, workflow QA, and integration QA strings. | QA public contracts have `QaFailureClassificationV1`; Director repair still emits a lowercase `polaris.qa_failure_classification.v1`; workflow/integration QA emit bare reasons. | Open | Introduce shared public QA classification builder and migrate Director repair/workflow/integration QA consumers to it. |
