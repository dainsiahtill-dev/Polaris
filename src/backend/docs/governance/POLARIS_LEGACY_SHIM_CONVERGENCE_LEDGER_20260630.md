...
| LS-01 | P1 | Open / partial | `polaris.application.orchestration` is marked `shim_only` in the migration ledger but still has production callers. | `src/backend/docs/migration/ledger.yaml` unit `mig-application-batch1`; remaining production imports from `delivery/cli/director/director_service.py` and `delivery/cli/director_v2.py`; `check_shim_markers.py --json` fails on 15 files. `delivery/cli/pm/director_interface_core.py` has been migrated to `ExecuteDirectorTaskCommandV1`. | Production imports move to Cell/RoleRuntime public contracts; `polaris/application/orchestration/` is deleted or reduced to fail-closed tombstones; `check_shim_markers.py --json` passes. | Migrate Director CLI iteration/batch paths next; preserve workflow submission behavior through `orchestration.workflow_runtime.public`. |
