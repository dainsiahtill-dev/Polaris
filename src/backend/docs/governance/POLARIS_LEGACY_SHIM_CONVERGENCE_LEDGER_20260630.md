...
| LS-01 | P1 | Open | `polaris.application.orchestration` is marked `shim_only` in the migration ledger but still has production callers. | `src/backend/docs/migration/ledger.yaml` unit `mig-application-batch1`; imports from `delivery/cli/pm/director_interface_core.py`, `delivery/cli/director/director_service.py`, `delivery/cli/director_v2.py`; `check_shim_markers.py --json` fails on 15 files. | Production imports move to Cell/RoleRuntime public contracts; `polaris/application/orchestration/` is deleted or reduced to fail-closed tombstones; `check_shim_markers.py --json` passes. | Migrate `DirectorOrchestrator` consumers first, then remove or mark remaining application orchestration files. |
