...
| Class | Count | Meaning |
| --- | ---: | --- |
| Closed in this convergence pass | 1 | Owner surfaces migrated to KFS and verified with focused tests. |
| P1 open | 4 | Execution, repair, verifier, or platform-control write paths close to runtime correctness. |
| P2 open | 4 | Internal Factory/bench, PM planning, instance, or benchmark write paths that still must converge. |
...
| KFS-08 | P2 | PM planning governance ledgers | `polaris/cells/orchestration/pm_planning/internal/decision_log.py`; `polaris/cells/orchestration/pm_planning/internal/milestones.py`; `polaris/cells/orchestration/pm_planning/internal/project_report.py`; `polaris/cells/orchestration/pm_planning/internal/raid_register.py` | Apply the CE blueprint ledger pattern: KFS atomic JSON writes, historical read compatibility retained. |
