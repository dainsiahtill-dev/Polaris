...
def test_cli_entrypoints_use_conditional_bootstrap(relative_path: str) -> None:
    ...
    assert "_bootstrap_backend_import_path" in source, (
        f"{relative_path} must define conditional path bootstrap helper"
    )
    assert "if __package__:" in source, (
        f"{relative_path} bootstrap must be conditional for package imports"
    )
    assert "scripts\") not in sys.path" not in source, (
        f"{relative_path} must not inject legacy scripts path"
    )
    assert "core\" / \"polaris_loop" not in source, (
        f"{relative_path} must not inject legacy core loop path"
    )
...
def test_pm_config_stays_import_side_effect_lightweight() -> None:
    ...
    assert "enforce_utf8()" not in source, (
        "pm.config must not mutate process stdio encoding at import time"
    )
    ...
    assert "get_settings()" not in source, (
        "pm.config must not resolve settings at import time"
    )
    assert "os.listdir(base_dir)" not in source, (
        "pm.config must avoid broad directory scans during import"
    )
...
def test_delivery_adapters_do_not_mutate_sys_path(relative_path: str) -> None:
    ...
    assert "sys.path.insert(" not in source, (
        f"{relative_path} must not mutate sys.path at runtime"
    )
