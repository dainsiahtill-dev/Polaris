...
class TestValidateToolStep:
    ...
    def test_repo_rg_with_valid_pattern(self) -> None:
        ...
        is_valid, error_code, error_msg = validate_tool_step("repo_rg", {"pattern": "test"})
    ...
    def test_repo_rg_with_pattern_and_path(self) -> None:
        ...
        is_valid, error_code, error_msg = validate_tool_step("repo_rg", {"pattern": "class", "path": "src/"})
        assert is_valid is True
        assert error_code is None
        assert error_msg == ""
    ...
    def test_repo_rg_alias_resolved(self) -> None:
        ...
        is_valid, error_code, _error_msg = validate_tool_step("grep", {"pattern": "test"})
        assert is_valid is True
        assert error_code is None
    ...
    def test_repo_read_head_with_valid_file(self) -> None:
        ...
        is_valid, error_code, error_msg = validate_tool_step("repo_read_head", {"file": "test.py"})
        assert is_valid is True
        assert error_code is None
        assert error_msg == ""
    ...
    def test_repo_read_head_with_valid_n(self) -> None:
        ...
        is_valid, error_code, error_msg = validate_tool_step("repo_read_head", {"file": "test.py", "n": 50000})
    ...
    def test_repo_read_head_alias_resolved(self) -> None:
        ...
        is_valid, error_code, _error_msg = validate_tool_step("read_file", {"file": "test.py"})
        assert is_valid is True
        assert error_code is None
    ...
    def test_repo_tree_valid(self) -> None:
        ...
        is_valid, error_code, _error_msg = validate_tool_step("repo_tree", {"path": "src/"})
        assert is_valid is True
        assert error_code is None

    def test_repo_diff_valid(self) -> None:
        """Test repo_diff with no required args passes validation."""
        is_valid, error_code, _error_msg = validate_tool_step("repo_diff", {})
        assert is_valid is True
        assert error_code is None

    def test_todo_read_valid(self) -> None:
        """Test todo_read with no args passes validation."""
        is_valid, error_code, _error_msg = validate_tool_step("todo_read", {})
        assert is_valid is True
        assert error_code is None
    ...
    def test_repo_rg_missing_pattern(self) -> None:
        ...
        is_valid, error_code, error_msg = validate_tool_step("repo_rg", {"path": "."})
    ...
    def test_repo_read_head_missing_file(self) -> None:
        ...
        is_valid, error_code, error_msg = validate_tool_step("repo_read_head", {"n": 10})
        assert is_valid is False
        assert error_code == "REQUIRED_MISSING"
    ...
    def test_repo_read_head_empty_file(self) -> None:
        ...
        is_valid, error_code, _error_msg = validate_tool_step("repo_read_head", {"file": "  "})
        assert is_valid is True
        assert error_code is None
    ...
    def test_unknown_tool(self) -> None:
        ...
        is_valid, error_code, error_msg = validate_tool_step("unknown_tool", {})
        assert is_valid is False
    ...
    def test_empty_tool_name(self) -> None:
        ...
        is_valid, error_code, _error_msg = validate_tool_step("", {})
        assert is_valid is False
        assert error_code == "UNKNOWN_TOOL"
    ...
    def test_background_run_negative_timeout(self) -> None:
        ...
        is_valid, error_code, error_msg = validate_tool_step("background_run", {"command": "echo hi", "timeout": -1})
        assert is_valid is False
    ...
    def test_background_run_zero_timeout(self) -> None:
        ...
        is_valid, error_code, error_msg = validate_tool_step("background_run", {"command": "echo hi", "timeout": 0})
        assert is_valid is False
        assert error_code == "INVALID_TOOL_ARGS"
        assert "greater than 0" in error_msg
    ...
    def test_background_run_exceeds_max_timeout(self) -> None:
        ...
        is_valid, error_code, error_msg = validate_tool_step("background_run", {"command": "echo hi", "timeout": 3601})
        assert is_valid is False
        assert error_code == "INVALID_TOOL_ARGS"
    ...
    def test_background_run_valid_max_timeout(self) -> None:
        ...
        is_valid, error_code, error_msg = validate_tool_step("background_run", {"command": "echo hi", "timeout": 3600})
        assert is_valid is True
        assert error_code is None
        assert error_msg == ""
    ...
    def test_background_run_default_timeout(self) -> None:
        ...
        is_valid, error_code, _error_msg = validate_tool_step("background_run", {"command": "echo hi"})
        assert is_valid is True
        assert error_code is None
    ...
    def test_none_args(self) -> None:
        ...
        is_valid, error_code, _error_msg = validate_tool_step("repo_rg", None)
    ...
    def test_non_dict_args(self) -> None:
        ...
        is_valid, error_code, _error_msg = validate_tool_step("repo_rg", "not a dict")
        # repo_rg requires pattern, so should fail
        assert is_valid is False
        assert error_code == "REQUIRED_MISSING"
