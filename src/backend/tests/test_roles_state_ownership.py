...
RUNTIME_CELL_YAML = (
    BACKEND_ROOT / "polaris" / "cells" / "roles" / "runtime" / "cell.yaml"
)
SESSION_CELL_YAML = (
    BACKEND_ROOT / "polaris" / "cells" / "roles" / "session" / "cell.yaml"
)
...
class TestSessionCellYaml:
    ...
    def test_state_owners_contains_role_sessions(self):
        owners: list = self.data.get("state_owners") or []
        assert "runtime/role_sessions/*" in owners, (
            "roles.session must own runtime/role_sessions/*"
        )

    def test_state_owners_contains_conversations(self):
        owners: list = self.data.get("state_owners") or []
        assert "runtime/conversations/*" in owners, (
            "roles.session must own runtime/conversations/*"
        )

    def test_state_owners_does_not_contain_runtime_roles(self):
        owners: list = self.data.get("state_owners") or []
        assert "runtime/roles/*" not in owners, (
            "roles.session must NOT own runtime/roles/* "
            "(that belongs to roles.runtime)"
        )

    def test_effects_allowed_does_not_write_runtime_roles(self):
        effects: list = self.data.get("effects_allowed") or []
        assert "fs.write:runtime/roles/*" not in effects, (
            "roles.session must NOT write runtime/roles/* "
            "(that is runtime Cell's responsibility)"
        )
...
class TestCatalogConsistency:
    ...
    def test_runtime_entry_exists_in_catalog(self):
        assert self.runtime_entry is not None, (
            "roles.runtime must be declared in cells.yaml catalog"
        )

    def test_session_entry_exists_in_catalog(self):
        assert self.session_entry is not None, (
            "roles.session must be declared in cells.yaml catalog"
        )

    def test_catalog_runtime_state_owners_no_role_sessions(self):
        owners: list = (self.runtime_entry or {}).get("state_owners") or []
        assert "runtime/role_sessions/*" not in owners, (
            "catalog: roles.runtime must NOT own runtime/role_sessions/*"
        )

    def test_catalog_session_state_owners_contains_role_sessions(self):
        owners: list = (self.session_entry or {}).get("state_owners") or []
        assert "runtime/role_sessions/*" in owners, (
            "catalog: roles.session must own runtime/role_sessions/*"
        )

    def test_catalog_session_state_owners_does_not_contain_runtime_roles(self):
        owners: list = (self.session_entry or {}).get("state_owners") or []
        assert "runtime/roles/*" not in owners, (
            "catalog: roles.session must NOT own runtime/roles/*"
        )

    def test_no_overlap_between_runtime_and_session_state_owners(self):
        runtime_owners: set = set(
            (self.runtime_entry or {}).get("state_owners") or []
        )
        session_owners: set = set(
            (self.session_entry or {}).get("state_owners") or []
        )
        overlap = runtime_owners & session_owners
        assert not overlap, (
            f"state_owners overlap between roles.runtime and roles.session: {overlap}"
        )

    def test_catalog_and_cell_yaml_runtime_state_owners_agree(self):
        catalog_owners: set = set(
            (self.runtime_entry or {}).get("state_owners") or []
        )
