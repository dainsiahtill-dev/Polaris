...
if nats_src.exists():
    shutil.move(str(nats_src), str(config_dir / "nats_config.py"))
if llm_src.exists():
    shutil.move(str(llm_src), str(config_dir / "llm_config.py"))
if pm_src.exists():
    shutil.move(str(pm_src), str(config_dir / "pm_config.py"))
if director_src.exists():
    shutil.move(str(director_src), str(config_dir / "director_config.py"))
...
content = content.replace(
    "from polaris.cells.director.internal.director_config import DirectorConfig",
    "from polaris.config.director_config import DirectorConfig",
)
content = content.replace(
    "from polaris.cells.orchestration.pm_planning.internal.pm_config import PMConfig",
    "from polaris.config.pm_config import PMConfig",
)
content = content.replace(
    "from polaris.kernelone.llm.internal.llm_config import LLMConfig", "from polaris.config.llm_config import LLMConfig"
)
content = content.replace(
    "from polaris.infrastructure.messaging.internal.nats_config import NATSConfig",
