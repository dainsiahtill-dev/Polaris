...
content = re.sub(r"from polaris\.cells\.policy\.workspace_guard\.public\.service import .*", "", content)

# Insert local import in apply_update
apply_update_pattern = (
    r'(def apply_update\(self, update: SettingsUpdate\) -> None:\n\s+"""Apply partial update payload\."""\n)'
)
content = re.sub(
    apply_update_pattern,
    r"\1        from polaris.cells.policy.workspace_guard.public.service import ensure_workspace_target_allowed\n",
    content,
)
...
content = re.sub(
    from_env_pattern,
    r"\1        from polaris.cells.policy.workspace_guard.public.service import ensure_workspace_target_allowed, SELF_UPGRADE_MODE_ENV\n",
    content,
)
