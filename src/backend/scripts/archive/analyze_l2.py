...
with open(path, encoding="utf-8") as f:
    for i, line in enumerate(f):
        ...
        if event == "llm_call_start":
            ...
            meta = d.get("metadata", {})
            print(
                f"   completion_tokens={d.get('completion_tokens')} tool_calls_count={d.get('tool_calls_count')} error={d.get('error_message')} finish={meta.get('finish_reason')} raw_tc={meta.get('stream_raw_tool_call_count')}"
            )
