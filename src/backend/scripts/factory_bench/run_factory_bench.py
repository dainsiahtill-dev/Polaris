...
_OPENCODE_AUDIT_AGENT_FOCI = (
    "runtime event 中失败角色的 tool_call/tool_result 原始证据",
    "最终给到 LLM 的 messages、final_request_context_audit 与 ContextOS snapshot",
    "ToolSpec、arg_aliases、工具名别名、参数字段归一化是否覆盖该模型调用习惯",
    "失败七分类归因：PM 合同、ChiefEngineer 蓝图、Director 工具执行、LLM 输出、上下文预算、目标项目基线、运行环境",
    "只读复核：是否存在 mock 成功、吞异常、静默 fallback、目标项目污染或产品实时轮询",
)


def _opencode_audit_execution_enabled() -> bool:
    raw = os.getenv("POLARIS_FACTORY_BENCH_RUN_OPENCODE_AUDITS", "1")
    return raw.strip().lower() not in {"0", "false", "no", "off", "skip", "disable", "disabled"}


def _opencode_audit_retry_enabled() -> bool:
    raw = os.getenv("POLARIS_FACTORY_BENCH_OPENCODE_AUDIT_RETRY_DATABASE_LOCK", "1")
    return raw.strip().lower() not in {"0", "false", "no", "off", "skip", "disable", "disabled"}
...
def _opencode_log_has_database_lock(log_path: Path) -> bool:
    try:
        ...
        return False
    lowered = text.lower()
    return "database is locked" in lowered or "sqlite_busy" in lowered


_OPENCODE_AUDIT_RUNTIME_FILES = (
    ...
    "events/qa.llm.events.jsonl",
    "qa/workspace-validation.json",
    "qa/report.json",
    "results/director.result.json",
    "dispatch/log.json",
)
_OPENCODE_AUDIT_INTERESTING_EVENT_TOKENS = (
    "failed",
    "failure",
    "task_failed",
    "tool_call",
...
def _trim_audit_value(value: Any, *, depth: int = 0, max_string: int = 2400) -> Any:
    if depth > 4:
    ...
    if isinstance(value, list):
        return [_trim_audit_value(item, depth=depth + 1, max_string=max_string) for item in value[:40]]
    if isinstance(value, dict):
        return {
            str(key): _trim_audit_value(item, depth=depth + 1, max_string=max_string)
            for key, item in list(value.items())[:80]
        }
    return _trim_audit_value(str(value), depth=depth, max_string=max_string)
...
def _record_workspace_hint(record: dict[str, Any], project_id: str) -> str:
    ...
    backend_workspace = ""
    if isinstance(metadata, dict):
        backend_workspace = str(metadata.get("workspace") or "").strip()
...
def _record_runtime_dirs(record: dict[str, Any]) -> list[Path]:
    candidates: list[str] = []
    ...
    if isinstance(runtime_dirs, list):
        candidates.extend(str(item) for item in runtime_dirs if str(item).strip())
...
def _event_details(data: dict[str, Any]) -> dict[str, Any]:
    raw_details = data.get("details")
    return raw_details if isinstance(raw_details, dict) else {}


def _event_metadata(payload: dict[str, Any], data: dict[str, Any]) -> dict[str, Any]:
...
def _extract_result_error_type(result: Any) -> str:
    if not isinstance(result, dict):
        return ""
    for key in ("error_type", "handler_error_type"):
        raw_value = result.get(key)
        if raw_value:
            return str(raw_value)
    raw_payload = result.get("payload")
    if isinstance(raw_payload, dict):
        for key in ("handler_error_type", "error_type"):
...
def _result_success(result: Any) -> bool | None:
    ...
    return raw_success if isinstance(raw_success, bool) else None


def _infer_role(role: str, run_id: str, claimed_by: str = "") -> str:
    if role:
        return role
    if claimed_by:
        return claimed_by
...
def _jsonl_event_view(payload: dict[str, Any], *, source: str = "", line_no: int = 0) -> dict[str, Any]:
    data = _event_data(payload)
    details = _event_details(data)
    metadata = _event_metadata(payload, data)
    result = data.get("result")
    role = _infer_role(
        str(payload.get("role") or data.get("role") or "").strip(),
        str(payload.get("run_id") or data.get("run_id") or metadata.get("run_id") or "").strip(),
        str(data.get("claimed_by") or "").strip(),
    )
    error = str(
        data.get("error") or data.get("error_message") or details.get("error") or _extract_result_error(result)
    ).strip()
    error_type = str(
        data.get("error_type") or data.get("handler_error_type") or details.get("error_type") or ""
    ).strip() or _extract_result_error_type(result)
    event = str(
        payload.get("event")
    ...
    status = str(data.get("status") or payload.get("status") or event).strip()
    return _trim_audit_value(
        {
            "source": source,
            "line_no": line_no,
            "ts": payload.get("ts") or payload.get("occurred_at") or data.get("timestamp"),
            "event_id": payload.get("event_id"),
            "event": event,
            "role": role,
            "run_id": payload.get("run_id") or data.get("run_id") or metadata.get("run_id"),
            "task_id": data.get("task_id") or metadata.get("task_id") or payload.get("aggregate_id"),
            ...
            "result": result,
            "details": details,
            "final_request_context_audit": metadata.get("final_request_context_audit"),
            "context_os_audit": metadata.get("context_os_audit"),
        }
    )


def _event_indicates_failure(view: dict[str, Any]) -> bool:
    event = str(view.get("event") or "").lower()
    status = str(view.get("status") or "").lower()
    error = str(view.get("error") or "").strip()
    ...
    if error:
        return True
    return any(token in event or token in status for token in ("failed", "failure", "error", "denied"))


def _compact_context_marker(view: dict[str, Any]) -> dict[str, Any]:
    return _trim_audit_value(
        {
            "ts": view.get("ts"),
            ...
            "final_request_context_audit": view.get("final_request_context_audit"),
            "context_os_audit": view.get("context_os_audit"),
        },
        max_string=1600,
    )


def _read_jsonl_payloads(path: Path) -> list[tuple[int, dict[str, Any]]]:
    try:
...
def _extract_role_tool_failure_events(runtime_dir: Path) -> list[dict[str, Any]]:
    contexts_by_run: dict[tuple[str, str], dict[str, Any]] = {}
    last_tool_calls: dict[tuple[str, str, str], dict[str, Any]] = {}
    ...
    for rel_path in llm_event_files:
        path = runtime_dir / rel_path
        if not path.is_file():
            continue
        for line_no, payload in _read_jsonl_payloads(path):
            view = _jsonl_event_view(payload, source=rel_path, line_no=line_no)
            role = str(view.get("role") or "").strip()
            run_id = str(view.get("run_id") or "").strip()
            tool = str(view.get("tool") or "").strip()
            key = (role, run_id)
            if isinstance(view.get("final_request_context_audit"), dict) or isinstance(
                view.get("context_os_audit"), dict
            ):
                contexts_by_run[key] = _compact_context_marker(view)
            if str(view.get("event") or "").strip() == "tool_call" and tool:
                last_tool_calls[(role, run_id, tool)] = _trim_audit_value(view, max_string=1600)
            if not _event_indicates_failure(view):
                continue
            failure = {
                **view,
                "audit_unit_type": "llm_role_event_failure",
                "nearest_final_request_context": contexts_by_run.get(key),
            }
            if tool:
                failure["nearest_tool_call"] = last_tool_calls.get((role, run_id, tool))
            failures.append(_trim_audit_value(failure, max_string=1800))

    task_runtime = runtime_dir / "events/task_runtime.execution.jsonl"
    if task_runtime.is_file():
        for line_no, payload in _read_jsonl_payloads(task_runtime):
            view = _jsonl_event_view(payload, source="events/task_runtime.execution.jsonl", line_no=line_no)
            if not _event_indicates_failure(view):
                continue
            role = str(view.get("role") or "").strip()
            run_id = str(view.get("run_id") or "").strip()
            failure = {
                **view,
                "audit_unit_type": "task_runtime_failure",
                "nearest_final_request_context": contexts_by_run.get((role, run_id)),
            }
            failures.append(_trim_audit_value(failure, max_string=1800))

    audited: list[dict[str, Any]] = []
    for index, failure in enumerate(failures[-80:], start=1):
        audited.append({"audit_unit_id": f"role-tool-failure-{index:03d}", **failure})
    return audited


def _read_audit_file_excerpt(path: Path) -> Any:
    try:
        text = path.read_text(encoding="utf-8", errors="replace")
    ...
    if path.suffix == ".json":
        try:
            return _trim_audit_value(json.loads(text))
        except json.JSONDecodeError:
            return {"text": _trim_audit_value(text)}
    if path.suffix == ".jsonl":
        entries: list[Any] = []
        for line in text.splitlines():
            lowered = line.lower()
            if not any(token in lowered for token in _OPENCODE_AUDIT_INTERESTING_EVENT_TOKENS):
                continue
            try:
                payload = json.loads(line)
            except json.JSONDecodeError:
                entries.append({"raw": _trim_audit_value(line)})
                continue
            entry = _jsonl_event_view(payload, source=path.name)
            entries.append(_trim_audit_value({key: value for key, value in entry.items() if value is not None}))
        return entries[-80:]
    return {"text": _trim_audit_value(text)}


def _build_opencode_audit_evidence_bundle(
    *,
    run_id: str,
    project_id: str,
    record: dict[str, Any],
    ...
    role_tool_failure_events: list[dict[str, Any]] = []
    for runtime_dir in _record_runtime_dirs(record):
        files: dict[str, Any] = {}
        for rel_path in _OPENCODE_AUDIT_RUNTIME_FILES:
            path = runtime_dir / rel_path
            if path.is_file():
                files[rel_path] = _read_audit_file_excerpt(path)
        runtime_evidence[str(runtime_dir)] = files
        role_tool_failure_events.extend(_extract_role_tool_failure_events(runtime_dir))
    ...
    return {
        "schema_version": "factory_bench.role_tool_failure_evidence.v2",
        "run_id": run_id,
        "project_id": project_id,
        "workspace": workspace,
        "runtime_dirs": [str(item) for item in _record_runtime_dirs(record)],
        "record_summary": _trim_audit_value({key: record.get(key) for key in summary_keys if key in record}),
        "target_workspace_evidence": _build_target_workspace_evidence(workspace, record),
        "runtime_evidence": runtime_evidence,
        "role_tool_failure_events": _trim_audit_value(role_tool_failure_events, max_string=1800),
        "audit_requirements": {
            "must_review": [
...
def _build_target_workspace_evidence(workspace: str, record: dict[str, Any]) -> dict[str, Any]:
    if not workspace:
        return {"available": False, "reason": "workspace_unknown"}
    workspace_path = Path(workspace)
    if not workspace_path.is_dir():
        return {"available": False, "reason": "workspace_not_found", "workspace": workspace}

    entries: list[dict[str, Any]] = []
    try:
        for child in sorted(workspace_path.iterdir(), key=lambda item: item.name.lower())[:80]:
            entries.append(
                {
                    "name": child.name,
        ...
        return {"available": False, "reason": f"{type(exc).__name__}: {exc}", "workspace": workspace}
    ...
    for rel_path in (
        ...
        if path.is_file():
            selected_files[rel_path] = _read_audit_file_excerpt(path)

    return {
        "available": True,
        "workspace": workspace,
        "top_level_entries": entries,
        "selected_config_files": selected_files,
        "record_code_files": _trim_audit_value(record.get("code_files") or []),
        "record_source_files": _trim_audit_value(record.get("source_files") or []),
    }


def _materialize_role_tool_failure_opencode_audit(
    *,
    base: Path,
    run_id: str,
    project_id: str,
    ...
    audit = record.get("opencode_audit")
    if not isinstance(audit, dict) or not audit.get("required"):
        return None

    request_dir = _OPENCODE_AUDIT_REQUEST_ROOT / _sanitize_run_id(run_id) / _sanitize_run_id(project_id)
    request_dir.mkdir(parents=True, exist_ok=True)
    evidence_bundle = _build_opencode_audit_evidence_bundle(run_id=run_id, project_id=project_id, record=record)
    evidence_path = request_dir / "evidence_bundle.json"
    evidence_path.write_text(
        json.dumps(evidence_bundle, ensure_ascii=False, indent=1) + "\n",
        encoding="utf-8",
    )
    evidence_rel_path = evidence_path.relative_to(_REPO_ROOT)
    raw_role_tool_failure_events = evidence_bundle.get("role_tool_failure_events")
    role_tool_failure_events = raw_role_tool_failure_events if isinstance(raw_role_tool_failure_events, list) else []
    role_tool_failure_event_count = len(role_tool_failure_events)
    role_tool_failure_unit_ids = [
        str(item.get("audit_unit_id"))
        for item in role_tool_failure_events
        if isinstance(item, dict) and str(item.get("audit_unit_id") or "").strip()
    ]
    ...
    for index, focus in enumerate(_OPENCODE_AUDIT_AGENT_FOCI[:agent_count], start=1):
        prompt = (
            f"{base_prompt}\n\n"
            f"可读审计证据包（相对主仓）：{evidence_rel_path}\n"
            f"可读审计证据包（绝对路径）：{evidence_path}\n"
            f"目标 workspace：{evidence_bundle.get('workspace') or '(unknown)'}\n"
            "必须优先读取该 evidence_bundle.json；路径只能使用上面的相对主仓路径或绝对路径，"
            "不要把证据路径拼接到目标 workspace 下，也不要直接用 OpenCode Read/cat 读取外部 cache/runtime/目标 workspace 目录。\n"
            "若 codegraph MCP 返回 database is locked，等待片刻后重试一次；仍失败时必须在 JSON 中标记审计工具故障。\n"
            f"证据包内 role_tool_failure_events 共 {role_tool_failure_event_count} 条；"
            "必须逐条审计每个 audit_unit_id，不能只审计项目级最终失败。\n"
            f"Agent {index:02d}/{agent_count:02d} focus: {focus}.\n"
            "硬性约束：只读审计；必须优先使用 codegraph MCP；必须检查最终 LLM 上下文和工具调用归一化；"
            "禁止修改 Polaris 或目标项目代码；最终只输出 JSON 审计报告。\n"
        )
        prompt_path = request_dir / f"agent_{index:02d}_prompt.txt"
        prompt_path.write_text(prompt, encoding="utf-8")
        prompt_paths.append(str(prompt_path))
        command_plan.append(["opencode", "run", f"$(cat {prompt_path})"])
        shell_commands.append(f'opencode run "$(cat {prompt_path})"')

    request_payload = {
        "schema_version": "factory_bench.role_tool_failure_opencode_audit.v1",
        "run_id": run_id,
        "project_id": project_id,
        "created_at": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
        "required": True,
        "source_audit_base": str(base),
        "reason": audit.get("reason"),
        "trigger_category": audit.get("trigger_category"),
        "root_cause_signature": audit.get("root_cause_signature"),
        "must_review": audit.get("must_review"),
        "forbidden": audit.get("forbidden"),
        "evidence_bundle_path": str(evidence_path),
        "repo_relative_evidence_bundle_path": str(evidence_rel_path),
    ...
    if not request_payload["execution_enabled"]:
        audit["execution_status"] = "request_materialized"
        return result

    opencode_bin = shutil.which("opencode")
    if not opencode_bin:
        audit["execution_status"] = "opencode_cli_missing"
