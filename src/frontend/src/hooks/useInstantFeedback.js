...
export function useInstantFeedback(options = {}) {
    const { successMessage = '操作成功', errorMessage = '操作失败', loadingMessage = '处理中...', showSuccessToast = true, showErrorToast = true, } = options;
...
export function useConfirmAction() {
    ...
    const confirm = useCallback(async (actionId, fn, options) => {
        // 防止重复确认
        if (isConfirming)
            return undefined;
