...
import { listBenchSessions, getBenchSession, } from '@/services/benchService';
import { useMessageHandler, useTransportActions } from '@/runtime/transport';
const MAX_EVENTS = 240;
function isTerminalStatus(status) {
    return status === 'completed' || status === 'failed' || status === 'cancelled';
}
function terminalStatusFromBenchEvent(eventType, meta) {
    const explicitStatus = typeof meta.status === 'string' ? meta.status.trim() : '';
    if (isTerminalStatus(explicitStatus))
        return explicitStatus;
    if (eventType === 'factory_bench.run.completed')
        return 'completed';
    if (eventType === 'factory_bench.run.failed')
        return 'failed';
    if (eventType === 'factory_bench.run.cancelled')
        return 'cancelled';
    return null;
}
function isBenchEnvelope(payload) {
    if (!payload)
        return false;
    if (typeof payload.channel === 'string' && payload.channel.startsWith('event.bench'))
        return true;
    if (typeof payload.kind === 'string' && payload.kind.startsWith('factory_bench.'))
        return true;
    return false;
}
...
function benchEventFromEnvelope(envelope) {
    if (!isBenchEnvelope(envelope))
        return null;
...
function workspaceFromBenchEvent(event) {
...
function mergeSessionSummary(existing, event) {
    const meta = event.meta || {};
    ...
    const controlPlaneProjection = readObject(meta.control_plane_projection)
...
function mergeBenchEvents(hydratedEvents, liveEvents) {
    const merged = [];
    const seen = new Set();
...
export function useFactoryBench(options = {}) {
    ...
    const notifyWorkspaceChange = useCallback((event) => {
    ...
    const loadSessionDetail = useCallback(async (sessionId, resetBeforeLoad, manualSelection = false) => {
        if (!enabled)
            return;
        if (loadingSessionRef.current === sessionId)
            return;
        if (!resetBeforeLoad && currentSessionRef.current?.session_id === sessionId)
            return;
        ...
        try {
            ...
            if (detailResult.ok && detailResult.data) {
                ...
                setEvents((prev) => {
                    if (resetBeforeLoad || detailEvents.length > 0)
                        return mergeBenchEvents(detailEvents, prev);
                    return prev;
                });
    ...
    const select = useCallback(async (sessionId) => {
    ...
    useEffect(() => {
        ...
        const unregister = registerMessageHandler((message) => {
            ...
            const isActiveSession = selectedSessionId === event.session_id ||
    ...
    useEffect(() => {
        if (!enabled)
            return;
        if (autoSelect !== 'newest')
            return;
        const newest = sessions[0];
        if (!newest)
            return;
        if (isTerminalStatus(newest.status) &&
            currentSession?.session_id === newest.session_id)
            return;
        if (selectedSessionRef.current === newest.session_id)
            return;
        if (currentSession && manualSelectionRef.current && !isTerminalStatus(currentSession.status))
            return;
        if (currentSession && currentSession.session_id === newest.session_id)
            return;
