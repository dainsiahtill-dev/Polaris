...
function streamNameFromPath(path) {
    return path.includes('/preview/') ? 'preview' : 'dialogue';
}
function createSessionId(streamName) {
    const cryptoId = globalThis.crypto?.randomUUID?.();
    if (cryptoId)
        return `docs-${streamName}-${cryptoId}`;
    return `docs-${streamName}-${Date.now()}-${Math.random().toString(36).slice(2, 10)}`;
}
function eventFromRuntimeMessage(message, channel) {
    const raw = message;
    const envelope = raw?.type === 'EVENT' ? raw.event : raw;
 ...
 */
export function useNDJSONStream(options = {}) {
    ...
    const startStream = useCallback(async (path, body) => {
        if (isStreaming)
            return;
        const streamName = streamNameFromPath(path);
        const requestBody = { ...body };
        const requestedSessionId = typeof requestBody.session_id === 'string' && requestBody.session_id.trim()
