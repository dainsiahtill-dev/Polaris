...
export function useLlmConfig(options = {}) {
    ...
    const queueSave = useCallback(async (newConfig) => {
    ...
    const updateRole = useCallback((role, updates) => {
        setConfig(prev => {
            if (!prev)
                return prev;
            const next = {
    ...
    const addProvider = useCallback((providerId, provider) => {
        setConfig(prev => {
            if (!prev)
                return prev;
            const next = {
