...
export function useAgentsReview(options) {
    ...
    const load = useCallback(async () => {
        if (!agentsReview || !agentsReview.needs_review)
            return;
        setLoading(true);
    ...
    const applyDraft = useCallback(async () => {
        if (!agentsReview?.needs_review || applying)
            return false;
        setApplying(true);
    ...
    const updateFeedback = useCallback((value) => {
        setFeedback(value);
