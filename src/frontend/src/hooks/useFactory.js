...
import { getFactoryRun, getFactoryRunArtifacts, listFactoryRuns, pauseFactoryRun, resumeFactoryRun, retryFactoryRunFromCheckpoint, startFactoryRun, stopFactoryRun, } from '@/services';
...
function factoryRunArtifactsKey(runId) {
    return ['factory', 'run', runId, 'artifacts'];
}
...
function runStatusFromFactoryEvent(runId, payload, previous) {
...
function mergeRunEvidenceFields(run, previous) {
...
export function useFactory(options = {}) {
    ...
    const [artifactsSnapshot, setArtifactsSnapshot] = useState(null);
    ...
    const factoryRunKey = (runId) => QueryKeys.factoryRun(runId);
    ...
    const startRunMutation = useMutation({
        ...
        onSuccess: (run) => {
            ...
            queryClient.setQueryData(factoryRunKey(run.run_id), run);
    ...
    const controlRunMutation = useMutation({
        mutationFn: async ({ runId, action, reason }) => {
            if (abortControllerRef.current) {
                abortControllerRef.current.abort();
            }
            abortControllerRef.current = new AbortController();
    ...
    const fetchRunArtifacts = useCallback(async (runId) => {
        ...
        try {
            ...
            if (result.ok && result.data) {
                ...
                queryClient.setQueryData(factoryRunArtifactsKey(normalizedRunId), snapshot);
    ...
    const loadRunSnapshot = useCallback(async (runId) => {
        ...
        try {
            ...
            if (result.ok && result.data) {
                ...
                setCurrentRun((previous) => mergeRunEvidenceFields(run, previous));
                // Update cache
                queryClient.setQueryData(factoryRunKey(runId), run);
                if (isTerminalRun(run)) {
    ...
    const connectStream = useCallback(async (runId) => {
        ...
        const handler = (message) => {
            ...
            if (eventChannel !== 'event.factory' && eventChannel !== channel)
                return;
            ...
            if (payload.run_id && (payload.status || typeof payload.progress === 'number')) {
                ...
                setCurrentRun((previous) => mergeRunEvidenceFields(run, previous));
                queryClient.setQueryData(factoryRunKey(eventRunId), run);
                if (isTerminalRun(run)) {
                    setIsStreaming(false);
                    queryClient.invalidateQueries({ queryKey: factoryRunsKey });
            ...
            const cached = queryClient.getQueryData(factoryRunKey(eventRunId)) || currentRun;
            ...
            queryClient.setQueryData(factoryRunKey(eventRunId), runPatch);
    ...
    const stopRun = useCallback(async (runId, reason) => {
        try {
    ...
    const controlRun = useCallback(async (runId, action, reason) => {
        try {
    ...
    const pauseRun = useCallback((runId, reason) => controlRun(runId, 'pause', reason), [controlRun]);
    const resumeRun = useCallback((runId, reason) => controlRun(runId, 'resume', reason), [controlRun]);
    const retryRunFromCheckpoint = useCallback(async (runId, reason) => {
    ...
    const fetchRuns = useCallback(async (limit = 20) => {
        ...
        try {
            ...
            if (result.ok && result.data) {
                // Update cache
                queryClient.setQueryData(factoryRunsKey, result.data);
                return result.data;
            }
            return [];
        }
    ...
    const activeArtifactsSnapshot = artifactsSnapshot?.run_id === currentRunId ? artifactsSnapshot : null;
    return {
        currentRun,
        events,
        isLoading: startRunMutation.isPending || stopRunMutation.isPending || controlRunMutation.isPending,
        error: (startRunMutation.error || stopRunMutation.error || controlRunMutation.error),
        isStreaming,
