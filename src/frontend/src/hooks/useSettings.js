...
export function useSettings(options = {}) {
    ...
    const stableSettingsRef = useRef(null);
    // Query for settings
    ...
    const getCachedSettings = () => {
        return queryClient.getQueryData(SETTINGS_QUERY_KEY) ?? null;
    };
    /**
    ...
    const update = async (updates) => {
        ...
        const currentData = queryClient.getQueryData(SETTINGS_QUERY_KEY);
        const optimisticData = { ...currentData, ...updates };
        queryClient.setQueryData(SETTINGS_QUERY_KEY, optimisticData);
        ...
        try {
            ...
            if (updateId === latestUpdateIdRef.current) {
                queryClient.setQueryData(SETTINGS_QUERY_KEY, stableSettingsRef.current ?? currentData ?? undefined);
            }
            throw error;
        }
    };
    return {
        ...
        setSettings: (newSettings) => {
            queryClient.setQueryData(SETTINGS_QUERY_KEY, newSettings ?? undefined);
        },
