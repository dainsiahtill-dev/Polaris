...
/** Provider 连接状态 */
export const useConnectivityStatus = (providerId) => useProviderStore((state) => state.providerTestStatus[providerId] || 'unknown');
/** Provider 是否展开 */
export const useIsProviderExpanded = (providerId) => useProviderStore((state) => state.expandedProviders.includes(providerId));
/** 当前编辑的 Provider ID */
export const useEditingProviderId = () => useProviderStore((state) => state.editingProviderId);
/** 编辑表单状态 */
export const useEditFormState = (providerId) => useProviderStore((state) => state.editFormState[providerId]);
/** 是否有未保存更改 */
export const useHasPendingChanges = (providerId) => useProviderStore((state) => state.pendingChanges.includes(providerId));
/** 是否正在保存 */
export const useIsSavingProvider = (providerId) => useProviderStore((state) => state.savingProvider === providerId);
/** Provider 错误 */
export const useProviderError = (providerId) => useProviderStore((state) => state.providerErrors[providerId]);
/** 全局未保存更改数量 */
export const useGlobalPendingChangesCount = () => useProviderStore((state) => state.pendingChanges.length);
