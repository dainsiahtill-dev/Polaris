 ...
 * 从 Run ID 解析迭代次数
 */
export function parseIterationFromRunId(runId) {
 ...
 * 从快照中获取迭代值
 */
export function toIterationValue(snapshot) {
 ...
 * 生成 Run Key
 */
export function toRunKey(snapshot) {
    if (!snapshot || typeof snapshot !== 'object')
        return '';
 ...
 * 判断是否为完成状态
 */
export function isDoneLikeStatus(value) {
 ...
 * 判断快照任务是否全部完成
 */
export function areSnapshotTasksDone(snapshot) {
 ...
 * 统计快照任务数量
 */
export function countSnapshotTasks(snapshot) {
 ...
 * 获取快照的 Director 状态
 */
export function getSnapshotDirectorStatus(snapshot) {
    const pmState = snapshot?.pm_state;
    if (!pmState || typeof pmState !== 'object') {
        return '';
    }
 ...
 * 判断是否应保留更丰富的快照
 */
export function shouldKeepRicherSnapshot(previous, incoming) {
