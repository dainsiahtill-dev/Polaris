 ...
 */
export function useUsageStats(llmStreamEvents) {
    const stats = useMemo(() => {
        const events = Array.isArray(llmStreamEvents) ? llmStreamEvents : [];
        if (events.length === 0)
            return null;
        const telemetry = buildTelemetryFromStream(events, [], []);
        if (!telemetry.hasData || (telemetry.totalCalls === 0 && telemetry.totalTokens === 0))
            return null;
        const byMode = Object.fromEntries(Object.entries(telemetry.byRole).map(([role, aggregate]) => [
            role,
