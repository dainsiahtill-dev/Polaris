...
import { getRuntimeProcessStreamKind, isProcessStreamChannel, normalizeDialogueEvent, } from '@/app/utils/appRuntime';
import { useSettings } from '@/hooks';
import { TaskStatus } from '@/types/task';
import * as Parsing from './runtimeParsing';
import { selectTaskRows, selectPrimaryStatus, isSystemActive, } from '@/runtime/projection';
...
function isInternalBenchEventChannel(channel) {
    return (channel === BENCH_EVENT_CHANNEL ||
        channel.startsWith(`${BENCH_EVENT_CHANNEL}:`));
}
function isRuntimeFactoryOrBenchEventChannel(channel) {
    return (channel === FACTORY_EVENT_CHANNEL ||
...
function normalizeRuntimeWorkspacePath(value) {
    const token = Parsing.toStringValue(value).trim();
    if (!token)
        return '';
    return token.replace(/\\/g, '/').replace(/\/+$/g, '');
}
function collectRuntimeWorkspacePaths(value, paths, seen, depth = 0) {
    if (depth > 5 || !Parsing.isRecord(value))
        return;
    if (seen.has(value))
        return;
    seen.add(value);
...
function runtimeRecordMatchesWorkspace(raw, activeWorkspace) {
    const active = normalizeRuntimeWorkspacePath(activeWorkspace);
    if (!active || !Parsing.isRecord(raw))
        return true;
    const candidates = new Set();
    collectRuntimeWorkspacePaths(raw, candidates, new WeakSet());
    if (candidates.size === 0)
        return true;
    return candidates.has(active);
}
function runtimeLineMatchesWorkspace(line, activeWorkspace) {
    const parsed = Parsing.tryParseJsonObject(line);
    return parsed ? runtimeRecordMatchesWorkspace(parsed, activeWorkspace) : true;
}
function isSettingsChangedEvent(raw) {
...
// ============================================================================
function toRuntimeEventPayload(payload) {
...
function normalizeRuntimeV2Envelope(eventPayload) {
    ...
    if (!targetChannel && v2Domain) {
        if (v2Domain === 'llm')
            targetChannel = 'llm';
        else if (v2Domain === 'process')
            targetChannel = 'process';
        else if (v2Domain === 'system')
...
function isRuntimeV2Envelope(payload) {
...
function residentStatusFromDomainPayload(payload) {
    const domainPayload = Parsing.isRecord(payload.payload) ? payload.payload : null;
    return firstRecord(domainPayload?.resident, domainPayload?.projection, Parsing.isRecord(domainPayload?.payload) ? domainPayload.payload.resident : null);
}
function mergeResidentIntoSnapshot(currentSnapshot, resident, timestamp) {
    return {
        ...(currentSnapshot ?? { timestamp: timestamp || new Date().toISOString() }),
        resident: resident,
    };
}
function normalizeTaskMatchToken(value) {
    return String(value || '').trim().toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-+|-+$/g, '');
}
function taskMatchTokens(task) {
...
function runtimeTaskLifecyclePayload(raw) {
    ...
    const hasLifecycleStateToken = eventText.includes('started') ||
    ...
    const isTaskLifecycle = eventText.includes('director_task') ||
    ...
    if (!isTaskLifecycle)
        return null;
    const taskId = Parsing.toStringValue(data?.task_id) ||
    ...
    else if (eventText.includes('blocked'))
        status = TaskStatus.BLOCKED;
    else if (eventText.includes('completed') || eventText.includes('success'))
        status = TaskStatus.COMPLETED;
    else if (eventText.includes('started') ||
        eventText.includes('running') ||
        eventText.includes('claimed') ||
        eventText.includes('in_progress')) {
    ...
    return {
        taskId,
        status,
        title: Parsing.toStringValue(data?.task_title) ||
...
function mergeRuntimeTaskLifecycle(tasks, update) {
    ...
    const nextTasks = tasks.map((task) => {
        if (!taskMatchTokens(task).includes(updateToken))
            return task;
        matched = true;
        return {
            ...task,
...
function extractLlmRunScope(parsed) {
...
function resolveLlmLogRunScope(log) {
...
function buildLlmDedupKey(log, fallbackRunScope) {
    const scopedRunId = resolveLlmLogRunScope(log) || fallbackRunScope || 'global';
    return `${scopedRunId}:${log.id}`;
}
function withLlmRunScope(log, fallbackRunScope) {
    const scopedRunId = resolveLlmLogRunScope(log) || fallbackRunScope || 'global';
    if (!scopedRunId || scopedRunId === 'global')
        return log;
...
function normalizeStreamEventToken(value) {
    return String(value || '').trim().toLowerCase();
}
function streamEventLabel(eventType) {
    switch (eventType) {
        case 'thinking_chunk':
            return '思考流';
...
function isChunkEvent(eventType) {
    return (eventType === 'thinking_chunk'
        || eventType === 'content_chunk'
        || eventType === 'thinking_preview'
        || eventType === 'content_preview');
}
function buildLlmMergeKey(log) {
    const meta = Parsing.isRecord(log.meta) ? log.meta : null;
...
function appendLlmStreamEntries(prev, incoming, limit) {
    if (!incoming.length)
        return prev;
    const next = [...prev];
...
function normalizeTaggedStreamLine(raw) {
...
function extractToolPayload(rawObj, eventData, parsed) {
    return firstRecord(rawObj?.payload, eventData?.payload, rawObj, eventData, parsed.tool || parsed.tool_name ? parsed : null);
}
function readToolName(toolPayload) {
    return Parsing.firstDisplayString(toolPayload?.tool, toolPayload?.tool_name, toolPayload?.name);
}
function readToolArgs(toolPayload) {
...
function readToolResult(toolPayload) {
    return firstRecord(toolPayload?.result, toolPayload?.output);
}
function readLlmContentText(rawObj, eventData, parsed) {
...
function normalizeContextSnapshotRef(value) {
    const token = String(value || '').trim();
    return CONTEXT_SNAPSHOT_REF_RE.test(token) ? token.toLowerCase() : '';
}
function parseLlmStreamLine(channel, line) {
    ...
    let details = '';
    if (parsed) {
        if (isLlmStreamChannel(channel) && !isLlmPayloadCompatible(parsed))
            return null;
        ...
        if (actor)
            source = Parsing.normalizeActorLabel(actor);
        const thinking = Parsing.firstDisplayString(parsed.thinking, parsed.content, parsed.message);
        const eventName = Parsing.toStringValue(parsed.event) ||
        ...
        const dataProviderId = Parsing.firstDisplayString(eventData?.provider_id, eventData?.providerId, dataMetadata?.provider_id, dataMetadata?.providerId, parsed.provider_id, parsed.providerId);
        const dataProviderName = Parsing.firstDisplayString(eventData?.provider_name, eventData?.providerName, eventData?.provider, dataMetadata?.provider_name, dataMetadata?.providerName, dataMetadata?.provider, parsed.provider_name, parsed.providerName, parsed.provider);
        const dataPromptTokens = positiveNumber(eventData?.prompt_tokens, eventData?.promptTokens, eventData?.input_tokens, eventData?.inputTokens, dataUsage?.prompt_tokens, dataUsage?.promptTokens, dataUsage?.input_tokens, dataUsage?.inputTokens, dataMetadata?.prompt_tokens, dataMetadata?.promptTokens, dataMetadata?.input_tokens, dataMetadata?.inputTokens);
        const dataCompletionTokens = positiveNumber(eventData?.completion_tokens, eventData?.completionTokens, eventData?.output_tokens, eventData?.outputTokens, dataUsage?.completion_tokens, dataUsage?.completionTokens, dataUsage?.output_tokens, dataUsage?.outputTokens, dataMetadata?.completion_tokens, dataMetadata?.completionTokens, dataMetadata?.output_tokens, dataMetadata?.outputTokens);
        const dataCacheCreationTokens = positiveNumber(eventData?.cache_creation_input_tokens, eventData?.cacheCreationInputTokens, dataUsage?.cache_creation_input_tokens, dataUsage?.cacheCreationInputTokens, dataMetadata?.cache_creation_input_tokens, dataMetadata?.cacheCreationInputTokens);
        const dataCacheReadTokens = positiveNumber(eventData?.cache_read_input_tokens, eventData?.cacheReadInputTokens, dataUsage?.cache_read_input_tokens, dataUsage?.cacheReadInputTokens, dataMetadata?.cache_read_input_tokens, dataMetadata?.cacheReadInputTokens);
        const dataCachedTokens = positiveNumber(eventData?.cached_tokens, eventData?.cachedTokens, eventData?.cached_prompt_tokens, dataUsage?.cached_tokens, dataUsage?.cachedTokens, dataUsage?.cached_prompt_tokens, dataMetadata?.cached_tokens, dataMetadata?.cachedTokens, dataCacheReadTokens);
        const dataReasoningTokens = positiveNumber(eventData?.reasoning_tokens, eventData?.reasoningTokens, dataUsage?.reasoning_tokens, dataUsage?.reasoningTokens, dataUsage?.output_tokens_details && typeof dataUsage.output_tokens_details === 'object'
            ? dataUsage.output_tokens_details.reasoning_tokens
            : undefined, dataUsage?.completion_tokens_details && typeof dataUsage.completion_tokens_details === 'object'
            ? dataUsage.completion_tokens_details.reasoning_tokens
            : undefined);
        const dataAudioTokens = positiveNumber(eventData?.audio_tokens, eventData?.audioTokens, dataUsage?.audio_tokens, dataUsage?.audioTokens);
        const dataToolTokens = positiveNumber(eventData?.tool_tokens, eventData?.toolTokens, dataUsage?.tool_tokens, dataUsage?.toolTokens);
        const dataTotalTokens = positiveNumber(eventData?.total_tokens, eventData?.totalTokens, dataUsage?.total_tokens, dataUsage?.totalTokens, dataMetadata?.total_tokens, dataMetadata?.totalTokens);
        const dataContextTokens = positiveNumber(eventData?.context_tokens_after, eventData?.contextTokens, eventData?.context_tokens_before, dataMetadata?.context_tokens_after, dataMetadata?.contextTokens, dataMetadata?.context_tokens_before);
        const dataContextSnapshotRef = normalizeContextSnapshotRef(Parsing.firstDisplayString(eventData?.context_snapshot_ref, eventData?.contextSnapshotRef, dataMetadata?.context_snapshot_ref, dataMetadata?.contextSnapshotRef, parsedRefs?.context_snapshot_ref, parsedRefs?.contextSnapshotRef));
        const dataContextSnapshotDegraded = firstRecord(eventData?.context_snapshot_degraded, eventData?.contextSnapshotDegraded, dataMetadata?.context_snapshot_degraded, dataMetadata?.contextSnapshotDegraded);
        const dataContextSnapshotDegradedReason = Parsing.firstDisplayString(eventData?.context_snapshot_degraded_reason, eventData?.contextSnapshotDegradedReason, dataMetadata?.context_snapshot_degraded_reason, dataMetadata?.contextSnapshotDegradedReason);
        const dataPromptHash = Parsing.firstDisplayString(eventData?.prompt_hash, eventData?.promptHash, dataMetadata?.prompt_hash, dataMetadata?.promptHash, parsedRefs?.prompt_hash, parsedRefs?.promptHash);
        const dataTurnId = Parsing.firstDisplayString(eventData?.turn_id, eventData?.turnId, dataMetadata?.turn_id, dataMetadata?.turnId, parsedRefs?.turn_id, parsedRefs?.turnId);
        const dataCallId = Parsing.firstDisplayString(eventData?.call_id, eventData?.callId, dataMetadata?.call_id, dataMetadata?.callId, parsedRefs?.call_id, parsedRefs?.callId);
        const dataFinalRequestContextAudit = firstRecord(eventData?.final_request_context_audit, eventData?.finalRequestContextAudit, dataMetadata?.final_request_context_audit, dataMetadata?.finalRequestContextAudit, parsed.final_request_context_audit, parsed.finalRequestContextAudit);
        const dataContextOSAudit = firstRecord(eventData?.context_os_audit, eventData?.contextOSAudit, dataMetadata?.context_os_audit, dataMetadata?.contextOSAudit, parsed.context_os_audit, parsed.contextOSAudit);
        ...
        else if (normalizedEvent === 'llm_completed' ||
...
function parseProcessStreamLine(channel, line) {
    const raw = Parsing.stripAnsi(String(line || '').trim());
    if (!raw)
        return null;
    const baseSource = (channel === 'system'
        ? 'System'
        : channel === 'process'
            ? 'Process'
    ...
    if (parsed) {
        ...
        if (channel === 'process' && canonicalChannel && canonicalChannel !== 'process')
            return null;
        if (channel === 'system' && canonicalChannel && canonicalChannel !== 'system')
            return null;
        ...
        if (parsedRole)
            source = Parsing.normalizeActorLabel(parsedRole);
        const eventName = Parsing.toStringValue(parsed.event) ||
            Parsing.toStringValue(parsed.name) ||
            Parsing.toStringValue(parsed.kind) ||
        ...
        const rawObj = parsed.raw && typeof parsed.raw === 'object' ? parsed.raw : null;
        ...
        streamEvent = (Parsing.toStringValue(dataObj?.stream_event) ||
        ...
        if (streamEvent === 'tool_call' || streamEvent === 'tool_result' || toolName) {
            ...
            if (level === 'info') {
                ...
                if (/error|failed|exception|traceback|timeout/.test(token))
                    level = 'error';
                else if (/warn|retry|blocked/.test(token))
                    level = 'warning';
                else if (/tool|invoke|llm|thinking|prompt/.test(token))
                    level = 'thinking';
                else if (/success|completed|done|passed/.test(token))
                    level = 'success';
            }
        }
    }
        ...
        if (/error|failed|exception|traceback|timeout/.test(text))
            level = 'error';
        else if (/warn|retry|blocked/.test(text))
            level = 'warning';
        else if (/tool|invoke|llm|thinking|prompt/.test(text))
            level = 'thinking';
        else if (/success|completed|done|passed/.test(text))
            level = 'success';
    }
...
function parseRuntimeEvent(raw) {
    ...
    const data = firstRecord(raw.data, raw.output, rawBody?.data, payload?.data, rawBody, payload) ?? {};
    ...
    const eventName = Parsing.toStringValue(raw.name) ||
    ...
    if (raw.error || raw.ok === false)
        ...
        level = 'warning';
    else if (eventName.includes('thinking') || eventName.includes('llm'))
        level = 'thinking';
    else if (raw.ok === true)
        level = 'success';
...
function parseQualityGateEvent(raw) {
    ...
    criticalIssues.forEach((msg) => {
        ...
        if (message)
            issues.push({ type: 'critical', message });
    });
...
export function useRuntime(options = {}) {
    const { roles = DEFAULT_RUNTIME_ROLES, baseUrl, autoConnect = true, maxRetries = Infinity, baseDelay = 1000, workspace: workspaceProp, includeInternalBench = false, tailLines, } = options;
    // Settings
    ...
    const { pmStatus, directorStatus, engineStatus, llmStatus, lancedbStatus, snapshot, anthroState, dialogueEvents, qualityGate, executionLogs, llmStreamEvents, processStreamEvents, currentPhase, fileEditEvents, tasks, workers, runId, taskProgressMap, taskTraceMap, sequentialTraceMap, setTasks, setQualityGate, setCurrentPhase, setRunId, setWorkers, setEngineStatus, setLlmStatus, setLancedbStatus, setSnapshot, setAnthroState, appendDialogueEvent, setDialogueEvents, appendExecutionLog, setExecutionLogs, appendLlmStreamEvent, setLlmStreamEvents, appendProcessStreamEvent, setProcessStreamEvents, updateTaskProgress, appendTaskTrace, appendSequentialTrace, appendFileEditEvent, resetForWorkspace, } = store;
    // Connection
    ...
    const seenDialogueIdsRef = useRef(new Set());
    const seenLlmEventIdsRef = useRef(new Set());
    const seenV2EventIdsRef = useRef(new Set());
    const llmRunScopeRef = useRef('global');
    ...
    const processMessage = useCallback((message) => {
        try {
            ...
            let payload = typeof eventData === 'string' ? JSON.parse(eventData) : eventData;
            ...
            if (msgType === 'event' && payload.event) {
                ...
                payload = normalizeRuntimeV2Envelope(payload);
                channel = String(payload.channel || '').trim();
            }
            ...
            const mergeTaskLifecycleFromRaw = (raw) => {
            ...
            if (payload.type === 'status') {
                ...
                if ('pm_status' in payload)
                    useRuntimeStore.getState().setPmStatus(payload.pm_status ?? null);
                if ('director_status' in payload)
                    useRuntimeStore.getState().setDirectorStatus(payload.director_status ?? null);
                if ('engine_status' in payload)
                    setEngineStatus(payload.engine_status ?? null);
                if ('llm_status' in payload)
                    setLlmStatus(payload.llm_status ?? null);
                if ('snapshot' in payload)
                    setSnapshot(payload.snapshot ?? null);
                if ('lancedb' in payload)
                    setLancedbStatus(payload.lancedb ?? null);
                if ('anthro_state' in payload)
                    setAnthroState(payload.anthro_state ?? null);
                ...
                setWorkers(Parsing.extractDirectorWorkers(payload.director_status ?? null));
                ...
                if (channel === 'dialogue') {
                    ...
                    payload.lines.forEach((line) => {
                        ...
                        try {
                            ...
                            if (!runtimeRecordMatchesWorkspace(raw, workspace))
                                return;
                    ...
                    payload.lines.forEach((line) => {
                        if (!line.trim())
                            return;
                        if (!runtimeLineMatchesWorkspace(line, workspace))
                            return;
                        try {
                            const raw = JSON.parse(line);
            ...
            if (payload.type === 'line' && payload.text) {
                if (channel === 'dialogue') {
                    try {
                        ...
                        if (normalized) {
                            const eventId = String(raw.event_id || '');
                            if (eventId && seenDialogueIdsRef.current.has(eventId))
                                return;
                            if (eventId) {
                    ...
                    if (!runtimeLineMatchesWorkspace(payload.text, workspace))
                        return;
