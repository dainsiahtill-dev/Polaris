...
export function useRuntimeLogs() {
     ...
     * 追加单个 LLM 流日志
     */
    const appendLlmLog = useCallback((log) => {
        appendLlmStreamEvent(log);
    }, [appendLlmStreamEvent]);
    /**
     * 批量更新 LLM 流日志
     */
    const updateLlmLogs = useCallback((logs) => {
        if (logs.length === 0)
            return;
     ...
     * 追加单个 Process 流日志
     */
    const appendProcessLog = useCallback((log) => {
        appendProcessStreamEvent(log);
    }, [appendProcessStreamEvent]);
    /**
     * 批量更新 Process 流日志
     */
    const updateProcessLogs = useCallback((logs) => {
        if (logs.length === 0)
            return;
     ...
     * 追加单个 Execution 日志
     */
    const appendExecutionLogEntry = useCallback((log) => {
        appendExecutionLog(log);
    }, [appendExecutionLog]);
    /**
     * 批量更新 Execution 日志
     */
    const updateExecutionLogs = useCallback((logs) => {
        if (logs.length === 0)
            return;
...
export function useTaskProgress() {
    ...
    const updateTask = useCallback((taskId, progress) => {
        updateTaskProgress(taskId, progress);
    }, [updateTaskProgress]);
    const getTaskProgress = useCallback((taskId) => taskProgressMap.get(taskId), [taskProgressMap]);
    return {
        tasks,
        setTasks,
        taskProgressMap,
...
export function useTaskTrace() {
    ...
    const getTaskTraces = useCallback((taskId) => taskTraceMap.get(taskId) || [], [taskTraceMap]);
    return {
        taskTraceMap,
        appendTaskTrace,
        getTaskTraces,
...
export function useSequentialTrace() {
    ...
    const getSequentialTraces = useCallback((runId) => sequentialTraceMap.get(runId) || [], [sequentialTraceMap]);
    return {
        sequentialTraceMap,
