...
import {
  MultiRoleInterviewStatus,
  InterviewDetailsModal,
} from "./MultiRoleInterviewStatus";
...
interface InterviewHallV2Props {
  ...
  onRunConnectivityTest: (payload: {
    role: RoleId;
    providerId: string;
    model: string;
  }) => void;
  onRunInterview: (payload: {
    role: RoleId;
    providerId: string;
    model: string;
  }) => void;
            ...
            return (
              ...
              >
                ...
                <div className="mt-3 text-[11px] text-text-main">
                  候选人：{role.candidate?.providerName || "未指派"}{" "}
                  {role.candidate?.model ? `• ${role.candidate.model}` : ""}
                </div>
              </button>
            );
          })}
        ...
        <div className="space-y-4">
                ...
                >
                  ...
                  <div className="text-[10px] text-text-dim text-right">
                    ...
                    <div>
                      ...
                      {candidate.thinkingConfidence !== null &&
          ...
          <div className="rounded-xl border border-white/10 bg-black/30 p-4 space-y-3">
            ...
            <div className="text-[11px] text-text-dim">
              思考检测：
              {activeRole?.thinkingConfidence !== null &&
...
function InterviewHallV2({
  ...
  const [inspectingProvider, setInspectingProvider] =
    useState<InterviewProviderSummary | null>(null);
  const activeRole = roles.find((role) => role.id === selectedRole);
  ...
  const connectivityNote =
    connectivity?.sourceRole && connectivity?.sourceRole !== activeRole?.id
  ...
  const canRunConnectivity = Boolean(
    activeRole && activeProvider && activeProviderModel,
  );
  const canRunInterview = Boolean(
    activeRole && activeProvider && activeProviderModel,
  );
          ...
          {roles.map((role) => {
            ...
            return (
              ...
              >
                ...
                <div className="mt-3 text-[11px] text-text-main">
                  默认人选: {role.candidate?.providerName || "未指定"}{" "}
                  {role.candidate?.model ? `• ${role.candidate.model}` : ""}
                </div>
              </button>
            );
                  ...
                  >
                    <div className="flex items-start justify-between gap-3">
                      <div>
                        <div className="flex items-center gap-2">
                          ...
                          <span
                            className={`text-[10px] uppercase tracking-wide px-2 py-0.5 rounded border ${styles.border} ${styles.text}`}
                          >
                      ...
                      <div className="flex flex-col items-end text-[10px] text-text-dim">
                        <span
                          className={`flex items-center gap-1 ${styles.text}`}
                        >
                          <span
                        ...
                        <span className="mt-1">
                          {formatTimestamp(
                            provider.lastConnectivityTest?.timestamp,
                          )}
                        </span>
                      </div>
                    </div>
                    {provider.lastConnectivityTest ? (
                      <div className="mt-3 text-[10px] text-text-dim">
                        延迟{" "}
                        {provider.lastConnectivityTest.latencyMs
                    ...
                    {provider.thinkingConfidence !== undefined &&
                    provider.thinkingConfidence !== null ? (
                      <div className="mt-2 text-[10px] text-text-dim">
                        思考置信度：
                        {Math.round(provider.thinkingConfidence * 100)}%
                        {provider.thinkingSupported === false
                          ? " (不支持)"
                          : ""}
                      </div>
                    ) : null}
                    {provider.lastInterview ? (
                      <div className="mt-2 text-[10px] text-text-dim">
                        ...
                        {provider.interviewStatus === "passed"
                          ? "通过"
                          : "未通过"}{" "}
                        • {formatTimestamp(provider.lastInterview.timestamp)}
                      </div>
                    ) : null}
                  </button>
                );
        ...
        <div className="space-y-4">
          ...
          <div className="rounded-xl border border-white/10 bg-black/30 p-4 space-y-4">
            <div className="space-y-2">
              <div className="text-xs text-text-main font-semibold">
                当前组合
              </div>
              <div className="text-[11px] text-text-dim">
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between text-[11px]">
                ...
                {connectivityRunning ? (
                  ...
                  <span className={connectivityColor}>{connectivityLabel}</span>
                )}
              </div>
              ...
              activeProvider ? (
                <button
                  type="button"
                  onClick={() =>
                    onSkipConnectivityTest(activeRole.id, activeProvider.id)
                  }
