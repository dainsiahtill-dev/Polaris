...
function normalizeStreamId(value, prefix) {
...
const START_EVENT_BY_TAG = {
    thinking: 'thinking_start',
    answer: 'answer_start',
};
const CHUNK_EVENT_BY_TAG = {
    thinking: 'thinking_chunk',
    answer: 'answer_chunk',
};
const END_EVENT_BY_TAG = {
    thinking: 'thinking_end',
    answer: 'answer_end',
};
const CLOSE_TAGS_BY_TAG = {
    thinking: ['</thinking>', '</think>', '</reasoning>', '</analysis>'],
    answer: ['</answer>', '</final>', '</response>'],
};
const normalizeTagName = (rawTag) => {
...
const trailingCloseTagPrefixLength = (value, tag) => {
...
export const createContentTagParser = (options = {}) => {
    let buffer = '';
    let activeTag = null;
    let untaggedAnswerOpen = false;
    const emit = (type, content, timestamp, onTagEvent) => {
        onTagEvent?.({
            type,
            data: {
                content,
    ...
    const emitUntaggedAnswer = (text, timestamp, onTagEvent) => {
        if (!options.emitUntaggedAsAnswer || !text)
            return;
    ...
    const consume = (chunk, timestamp, onTagEvent) => {
        if (!chunk)
            return;
        buffer += chunk;
    ...
    const flush = (timestamp, onTagEvent) => {
        if (activeTag) {
            if (buffer) {
...
export function useInterviewStream(options = {}) {
    ...
    const unregisterHandlerRef = useRef(null);
    ...
    const requestCancel = useCallback(async (sessionId) => {
        try {
        ...
        try {
            ...
            const handleStreamEvent = (eventType, data) => {
                switch (eventType) {
                    ...
                    case 'agent_message': {
                        const itemId = typeof data.item_id === 'string' || typeof data.item_id === 'number'
                            ? String(data.item_id)
                            : '';
                        const event = {
                            id: itemId || `${eventType}-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`,
                            kind: eventType === 'thinking'
                                ? 'reasoning'
                                : eventType,
                    ...
                    case 'complete':
                        contentTagParser.flush(typeof data.timestamp === 'string' ? data.timestamp : new Date().toISOString(), onTagEvent);
                        finalResult = {
                            ...data,
    ...
    const stopStream = useCallback((sessionIdOverride) => {
