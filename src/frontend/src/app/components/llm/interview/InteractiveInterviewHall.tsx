...
import {
  useInterviewStream,
  type RealtimeThinkingEvent,
  type StreamingTagEvent,
} from "./useInterviewStream";
import {
  resolveModelName,
  validateModelName,
...
const QUESTION_TEMPLATES: QuestionTemplate[] = [
  {
    ...
    title: "项目需求分析",
    question:
      "请分析这个项目需求并制定实施计划，包括时间安排、资源分配和风险评估。",
  ...
  },
  {
    ...
    title: "技术分歧协调",
    question:
      "开发团队在前端技术选型上出现分歧，作为PM你如何协调解决？请说明具体步骤和考虑因素。",
  ...
  },
  {
    ...
    title: "架构方案选择",
    question:
      "如果需要在稳定性和交付速度之间权衡，你会如何做架构决策？请给出判断依据。",
  ...
  },
  {
    ...
    title: "无人值守决策边界",
    question:
      "当平台无人值守运行时，你如何判断一个修复建议是否只能作为 advisory，而不能直接转成写入决策？请给出证据链和阻断条件。",
  ...
  },
  {
    ...
    title: "AGI 审计交接",
    question:
      "如果 Director 进入修复阶段且确定性策略覆盖不足，你会如何调用平台能力完成审计、建议和交接，同时避免越权执行？",
...
export function InteractiveInterviewHall({
  ...
  const syncTestPanelStatus = useCallback(
    (status: "idle" | "running" | "success" | "failed") => {
  ...
  const hasStreamingContent =
    streamingThinking || streamingAnswer || isThinkingActive || isAnswerActive;
  ...
  const hasPendingEvaluation = answerMessages.some(
    (message) =>
      !message.evaluation || message.evaluation.userRating === "pending",
  );
  const passedAnswers = answerMessages.filter(
    (message) => message.evaluation?.userRating === "pass",
  ).length;
  const canFinalize =
    answerMessages.length > 0 && !hasPendingEvaluation && !responding;
  ...
  const showFloatingTemplatePanel =
    isDeepTestMode && showTemplatePanel && !isFullscreen;
  ...
  useEffect(() => {
    ...
    setLeftPanelCollapsed(
      isDeepTestMode && Boolean(selectedRole && selectedProvider),
    );
  }, [
    clearTagEvents,
    clearThinkingEvents,
    ...
    try {
      ...
      const answerMessage: InterviewMessage = {
        ...
        evaluation: {
          userRating: "pending",
          notes: "",
          criteriaAssessment: normalizeCriteriaAssessment(
            template?.expectedCriteria || [],
          ),
        },
      };
      setMessages((prev) => [...prev, answerMessage]);
  ...
  const updateEvaluation = (
    messageId: string,
    updates: Partial<InterviewMessage["evaluation"]>,
  ) => {
    setMessages((prev) =>
      prev.map((message) => {
  ...
  const buildReport = (
    ...
    return {
      ...
      summary: {
        ...
        weaknesses,
        recommendation:
          overallStatus === "passed" ? "建议通过面试" : "建议进一步提升后重试",
      },
      userNotes,
    };
  };
  ...
  if (view === "report" && report) {
    return (
      <div
        data-testid="llm-interactive-report"
        className="soft-panel rounded-xl p-5 space-y-4"
      >
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <h3 className="text-lg font-semibold text-text-main">
              面试报告 · {activeRole?.label || report.role}
            </h3>
        ...
        {saving ? (
          ...
          <div
            data-testid="llm-interactive-report-saved-path"
            className="text-[11px] text-emerald-300"
          >
            报告已保存：{reportSavedPath}
          </div>
        ) : null}
        {error ? (
        ...
        <div className="soft-panel-subtle rounded-xl p-4 text-xs space-y-2">
          ...
          <div className="text-text-main">
            推荐：{report.summary.recommendation}
          </div>
          ...
          {report.summary.weaknesses.length > 0 ? (
            <div className="text-text-dim">
              待提升：{report.summary.weaknesses.join("、")}
            </div>
          ) : null}
          {report.userNotes ? (
  ...
  return (
    ...
    >
      {/* Header */}
      <div
        className={`soft-panel rounded-xl px-3 ${compactMode ? "py-1" : "py-2"}`}
      >
        <div className="flex flex-wrap items-center justify-between gap-2">
          ...
          <div className="flex flex-wrap items-center gap-2 text-[10px]">
            <span
              className={`px-2 py-1 rounded border uppercase tracking-wide ${SESSION_STATUS[sessionStatus].badge}`}
            >
      ...
      >
        ...
        {showLeftPanel ? (
          <div
            className={`${compactMode ? "grid grid-rows-[auto_1fr] gap-2" : "grid grid-rows-[auto_1fr_auto_1fr] gap-4"} min-h-0 min-w-0 overflow-hidden`}
          >
            ...
            {leftPanelCollapsed && isDeepTestMode ? (
              <div className="space-y-2">
                <div className="soft-inset rounded-lg p-2 text-[10px]">
                  ...
                  <div className="text-text-main mt-1">
                    {activeRole?.label || "未选择"}
                  </div>
                </div>
                <div className="soft-inset rounded-lg p-2 text-[10px]">
                  ...
                  <div className="text-text-dim mt-1">
                    {selectedModel || activeProvider?.model || "未设置模型"}
                  </div>
                </div>
              </div>
            ) : (
              <>
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-2">
                            <span
                              className={`px-2 py-1 text-[10px] uppercase font-semibold rounded border ${badge}`}
                            >
                              {role.label}
                            </span>
                ...
                <div className="space-y-2 min-h-0 overflow-auto pr-1">
                  {providers.map((provider) => {
                    ...
                    const styles =
                      STATUS_STYLES[provider.status] || STATUS_STYLES.untested;
                    return (
                      <button
                        key={provider.id}
                      ...
                      >
                        <div className="flex items-center justify-between">
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <span
                              className={`text-[9px] uppercase px-2 py-0.5 rounded border ${styles.border} ${styles.text}`}
                            >
            ...
            <div className="space-y-3 flex-1 min-h-0 overflow-auto pr-1">
              {templatesByCategory.length === 0 ? (
                ...
                templatesByCategory.map(([category, templates]) => (
                  <div
                    key={category}
                    className="soft-chip rounded-xl p-3 space-y-2"
                  >
        ...
        <div className="flex min-h-0 min-w-0 flex-1 basis-0 flex-col overflow-hidden xl:order-2">
          ...
          >
            <div
              className={`${compactMode ? "sr-only" : "flex"} shrink-0 items-center justify-between gap-2 text-[11px] text-text-dim`}
            >
            ...
            {showThinkingPanel ? (
              <div
                data-testid="llm-interactive-stream-monitors"
                className={`grid min-h-[84px] min-w-0 shrink-0 gap-2 overflow-hidden ${
                  compactMode
            ...
            {messages.length === 0 ? (
              <div
                data-testid="llm-interactive-messages"
                className="flex min-h-[120px] min-w-0 flex-1 items-center justify-center soft-inset rounded-lg text-xs text-text-dim"
              >
              ...
              <div
                data-testid="llm-interactive-messages"
                className={`min-h-[120px] min-w-0 flex-1 overflow-y-auto overscroll-contain soft-inset rounded-lg custom-scrollbar ${compactMode ? "space-y-2 p-2 pr-1" : "space-y-3 p-3 pr-2"}`}
              >
                {qaPairs.map((pair, index) => {
                  const question = pair.question;
                  const answer = pair.answer;
                  const criteria =
                    answer?.expectedCriteria ||
                  ...
                  return (
                    ...
                    >
                      ...
                      {question ? (
                        <div
                          className={`soft-inset rounded-lg ${compactMode ? "p-2" : "p-3"}`}
                        >
                          <div className="text-[10px] uppercase tracking-wide text-text-dim mb-1">
                            提问
                          </div>
                      ...
                      {answer ? (
                        <div
                          className={`soft-inset rounded-lg ${compactMode ? "p-2 space-y-1.5" : "p-3 space-y-2"}`}
                        >
                          <div className="text-[10px] uppercase tracking-wide text-text-dim mb-1">
                            作答
                          </div>
                              ...
                              <div className="space-y-1 text-[10px] text-text-dim">
                                ...
                                <div className="flex flex-wrap gap-x-3 gap-y-1">
                                  {criteria.map((item) => (
                                    <label
                                      key={item}
                                      className="flex items-center gap-1 cursor-pointer hover:text-text-main"
                                    >
                                      <input
                                        type="checkbox"
                                        checked={Boolean(
                                          answer.evaluation
                                            ?.criteriaAssessment?.[item],
                                        )}
                                        onChange={(event) => {
                                          updateEvaluation(answer.id, {
                                            criteriaAssessment: {
                        ...
                        <div className="text-[11px] text-text-dim">
                          {responding && !hasStreamingContent
                            ? "等待回答中..."
                            : !responding
                              ? "暂无回答"
                              : null}
            ...
            <div
              data-testid="llm-interactive-composer"
              className={`soft-inset rounded-lg flex-shrink-0 ${compactMode ? "grid grid-cols-[minmax(0,1fr)_120px] items-end gap-2 p-2" : "p-3 space-y-2"}`}
            >
              <div
                className={
                  compactMode
            ...
            {!isFullscreen && compactMode ? (
              <div
                data-testid="llm-interactive-finalize-controls"
                className="flex shrink-0 flex-wrap items-center gap-2 soft-inset rounded-lg p-1.5"
              >
                <input
                  value={userNotes}
          ...
          {!isFullscreen && !compactMode ? (
            <div
              data-testid="llm-interactive-finalize-controls"
              className={`soft-panel-subtle rounded-xl flex-shrink-0 ${compactMode ? "p-2.5 space-y-2" : "p-4 space-y-3"}`}
            >
              <div className="text-xs font-semibold text-text-main uppercase tracking-wide">
                面试控制
              </div>
              ...
              />
              <div
                className={`flex ${compactMode ? "flex-row flex-wrap" : "flex-col"} gap-2`}
              >
                <button
                  data-testid="llm-interactive-finalize-pass"
                  onClick={() => finalizeInterview("passed")}
      ...
      {showFloatingTemplatePanel ? (
        <div className="absolute right-2 top-14 z-40 w-[min(420px,94vw)] soft-panel rounded-xl p-3">
          <div className="flex items-center justify-between mb-2">
            <div className="text-[10px] uppercase tracking-wide text-text-main">
              模板问题面板
            </div>
            <button
              type="button"
          ...
          <div className="space-y-2 max-h-64 overflow-y-auto pr-1">
            {templatesByCategory.length === 0 ? (
              ...
              templatesByCategory.map(([category, templates]) => (
                <div
                  key={category}
                  className="soft-inset rounded p-2 space-y-2"
                >
