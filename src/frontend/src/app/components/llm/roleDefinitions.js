...
export const LLM_ROLE_DEFINITIONS = {
    pm: {
        id: "pm",
        label: "PM",
    ...
    scout: {
        id: "scout",
        label: "Scout",
        description: "只读代码/文档侦察（探子）。可选：仅当 scout_probe 升级为 LLM 侦察时才需要绑定模型。",
        bindingVisibility: "default",
        bindingKind: "execution_role",
        requiresThinking: false,
        minConfidence: 0.5,
...
export const GOVERNANCE_ADVISOR_LLM_ROLE_IDS = LLM_ROLE_IDS.filter((roleId) => LLM_ROLE_DEFINITIONS[roleId].bindingKind === "governance_advisor");
export const DEFAULT_LLM_BINDING_ROLE_IDS = LLM_ROLE_IDS.filter((roleId) => LLM_ROLE_DEFINITIONS[roleId].bindingVisibility === "default");
export const OPTIONAL_GOVERNANCE_LLM_ROLE_IDS = LLM_ROLE_IDS.filter((roleId) => LLM_ROLE_DEFINITIONS[roleId].bindingVisibility === "configured");
export const REQUIRED_LLM_ASSIGNMENT_ROLE_IDS = [
    ...
    "qa",
    "architect",
], satisfies, readonly, LlmRoleId, [];
export function isKnownLlmRoleId(value) {
    return Object.prototype.hasOwnProperty.call(LLM_ROLE_DEFINITIONS, value);
}
export function normalizeLlmRoleId(value) {
    if (value === "docs")
        return "architect";
    return isKnownLlmRoleId(value) ? value : null;
}
export function normalizeLlmRoleIds(values) {
    const roleIds = [];
...
export function getRequiredLlmAssignmentRoleIds(policies) {
    return [
        ...new Set([
...
export function getLlmRoleDefinition(roleId) {
    return LLM_ROLE_DEFINITIONS[roleId];
}
function hasConfiguredRoleBinding(value) {
...
export function getVisibleLlmBindingRoleIds(roles, statusRoles) {
    const optionalRoles = OPTIONAL_GOVERNANCE_LLM_ROLE_IDS.filter((roleId) => Boolean(hasConfiguredRoleBinding(roles?.[roleId]) || statusRoles?.[roleId]));
