...
import { useProviderActions, useProviderState, useEditingProviderId, useEditFormState, useHasPendingChanges, useIsSavingProvider, useProviderError, } from './ProviderContext';
/**
 * useProviderForm Hook
 *
 * 使用示例:
 ...
 * ```
 */
export function useProviderForm(options) {
    const { providerId, initialConfig, onSave, onValidate } = options;
    const { startEdit, updateEditForm, saveEditStart, saveEditSuccess, saveEditFailure, cancelEdit, clearProviderError, } = useProviderActions();
    // 从 Context 获取状态
    ...
    const handleUpdateField = useCallback((field, value) => {
        updateEditForm(providerId, { [field]: value });
    }, [providerId, updateEditForm]);
    // 批量更新字段
    const handleUpdateFields = useCallback((updates) => {
