 ...
 * 4. unknown - 默认状态
 */
export function calculateConnectivityStatus(isRunning, backendResult, cachedResult, cacheTtlMs = CACHE_TTL_MS) {
 ...
 * 获取连接性状态的时间戳
 */
export function getConnectivityTimestamp(backendResult, cachedResult) {
    // 优先使用后端时间戳
 ...
 * 检查缓存是否过期
 */
export function isCacheExpired(cachedResult, cacheTtlMs = CACHE_TTL_MS) {
    if (!cachedResult)
        return true;
    const cacheAge = Date.now() - new Date(cachedResult.timestamp).getTime();
 ...
 * 获取连接性状态的颜色类
 */
export function getConnectivityStatusColor(status) {
    switch (status) {
        case 'running':
 ...
 * 获取连接性状态的背景色类
 */
export function getConnectivityStatusBgColor(status) {
    switch (status) {
        case 'running':
 ...
 * 获取连接性状态的指示器颜色类
 */
export function getConnectivityStatusDotColor(status) {
    switch (status) {
        case 'running':
