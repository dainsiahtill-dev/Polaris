...
describe("visual copy sync", () => {
  ...
  it("uses updated role and provider wording in validation copy", () => {
    ...
    expect(issues.some((issue) => issue.message.includes("Provider"))).toBe(
      false,
    );
  ...
  it("requires Resident AGI in visual validation only when the policy enables it", () => {
    const baseline = validateVisualGraph({ providers: {}, roles: {} });
    expect(
      baseline.issues.some((issue) => issue.nodeId === "role:resident_agi"),
    ).toBe(false);

    const required = validateVisualGraph({
      providers: {},
      roles: {},
      policies: { required_ready_roles: ["resident_agi"] },
    });
    expect(
      required.issues.some((issue) => issue.nodeId === "role:resident_agi"),
    ).toBe(true);
  });
});
