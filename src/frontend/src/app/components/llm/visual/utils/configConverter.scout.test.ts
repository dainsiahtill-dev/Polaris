...
describe("LLM visual config editor — Scout role", () => {
  it("renders a Scout role node in the visual graph", () => {
    ...
    expect(String((scoutNode?.data as { label?: string }).label || "")).toBe(
      "Scout",
    );
  });
  ...
  it("persists a scout provider/model assignment and round-trips to runtime config", () => {
    ...
    expect(next.roles?.scout).toEqual(
      expect.objectContaining({ provider_id: "openai", model: "gpt-5" }),
    );
    ...
    const scoutAssignment = runtime.roleAssignments.find(
      (assignment) => assignment.roleId === "scout",
    );
...
describe("LLM visual config editor — Resident AGI role", () => {
  it("renders a Resident AGI role node in the visual graph", () => {
    const { nodes } = buildVisualGraph({ providers: {}, roles: {} });
    const agiNode = nodes.find((node) => node.id === "role:resident_agi");
    expect(agiNode).toBeDefined();
    expect(agiNode?.type).toBe("role");
    expect(String((agiNode?.data as { label?: string }).label || "")).toBe(
      "Resident AGI",
    );
  });

  it("labels resident_agi consistently across the editor helpers", () => {
    expect(getRoleLabel("resident_agi")).toBe("Resident AGI");
    expect(getRoleDisplayLabel("resident_agi")).toBe("Resident AGI");
  });

  it("persists a Resident AGI provider/model assignment and round-trips to runtime config", () => {
    const base: VisualGraphConfig = {
      providers: { openai: { type: "openai", name: "OpenAI" } },
      roles: {},
    };

    const next = updateRoleAssignment(
      base,
      "resident_agi",
      "openai",
      "gpt-5.5",
    );
    expect(next.roles?.resident_agi).toEqual(
      expect.objectContaining({ provider_id: "openai", model: "gpt-5.5" }),
    );

    const runtime = visualToRuntimeConfig(next);
    const agiAssignment = runtime.roleAssignments.find(
      (assignment) => assignment.roleId === "resident_agi",
    );
    expect(agiAssignment).toBeDefined();
    expect(agiAssignment?.providerId).toBe("openai");
    expect(agiAssignment?.model).toBe("gpt-5.5");
  });

  it("treats Resident AGI as optional until the AGI switch requires it", () => {
    const result = validateRoleAssignments({ providers: {}, roles: {} });
...
describe("LLM visual config editor — optional governance roles", () => {
  it("does not show CFO or HR as default LLM binding roles", () => {
    const { nodes } = buildVisualGraph({ providers: {}, roles: {} });
    const roleIds = nodes
      .filter((node) => node.type === "role")
      .map((node) => node.id);

    expect(roleIds).not.toContain("role:cfo");
    expect(roleIds).not.toContain("role:hr");
  });

  it("keeps configured CFO and HR bindings visible for backward compatibility", () => {
    const config: VisualGraphConfig = {
      providers: { openai: { type: "openai", name: "OpenAI" } },
      roles: {
        cfo: { provider_id: "openai", model: "gpt-4.1-mini" },
        hr: { provider_id: "openai", model: "gpt-4.1-mini" },
      },
    };
    ...
    const roleIds = nodes
      .filter((node) => node.type === "role")
      .map((node) => node.id);
    const modelRoleEdges = edges.filter(
      (edge) => edge.data?.kind === "model-to-role",
    );

    expect(roleIds).toContain("role:cfo");
    expect(roleIds).toContain("role:hr");
    expect(modelRoleEdges.some((edge) => edge.target === "role:cfo")).toBe(
      true,
    );
    expect(modelRoleEdges.some((edge) => edge.target === "role:hr")).toBe(true);
  });
});
...
describe("LLM visual config editor — multi-binding concurrency", () => {
  it("adds multiple model bindings to one role without replacing the first binding", () => {
    ...
    const first = updateRoleAssignment(base, "director", "kimi", "kimi-k2", {
      maxConcurrency: 3,
    });
    const second = updateRoleAssignment(first, "director", "kimi", "kimi-k1", {
      maxConcurrency: 2,
    });
  ...
  it("converts every role binding to runtime assignments with concurrency metadata", () => {
    ...
    expect(
      runtime.roleAssignments.filter(
        (assignment) => assignment.roleId === "director",
      ),
    ).toEqual([
      {
        roleId: "director",
  ...
  it("updates provider, role, and binding concurrency knobs independently", () => {
    ...
    const bindingCapped = updateRoleBindingConcurrency(
      roleCapped,
      "director",
      "kimi",
      "kimi-k2",
    ...
    expect(bindingCapped.providers.kimi).toEqual(
      expect.objectContaining({ max_concurrency: 20 }),
    );
