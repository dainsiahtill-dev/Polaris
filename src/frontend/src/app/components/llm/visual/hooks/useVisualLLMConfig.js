...
import { addManualModel, buildVisualGraph, clearRoleAssignment, extractNodePositions, mergeNodePositionsWithStates, modelNodeId, removeManualModel, removeProvider, removeRoleBinding, restoreNodeStates, updateRoleAssignment, updateVisualStates, } from '../utils/configConverter';
const isSameRuntimeStatus = (previous, next) => {
    if (!previous && !next)
        return true;
    if (!previous || !next)
        return false;
...
const applyRuntimeStatusToNodes = (currentNodes, runtimeStatus) => {
...
export function useVisualLLMConfig({ config, status, onConfigChange }) {
    const graph = useMemo(() => {
        if (!config)
            return { nodes: [], edges: [] };
    ...
    // 同步节点位置到配置
    const syncNodePositions = useCallback((currentConfig, currentNodes) => {
    ...
    // 同步完整的节点状态到配置
    const syncNodeStates = useCallback((currentConfig, currentNodes, currentEdges) => {
        const onChange = onConfigChangeRef.current;
        if (!onChange)
            return;
    ...
    const updateConfigRole = useCallback((roleId, providerId, model) => {
    ...
    const clearConfigRole = useCallback((roleId) => {
        const currentConfig = latestConfigRef.current;
        const onChange = onConfigChangeRef.current;
        if (!currentConfig || !onChange)
            return;
        ...
        if (sourceNode.type === 'model' && targetNode.type === 'role') {
            ...
            if (providerData.kind === 'provider' && roleData.kind === 'role') {
                ...
                if (currentConfig && currentConfig.providers) {
                    const providerCfg = currentConfig.providers[providerId];
                ...
                if (!targetModel) {
                    const existingModelNode = nodes.find((n) => n.type === 'model' && n.data.kind === 'model' && n.data.providerId === providerId);
    ...
    const onEdgesDelete = useCallback((deleted) => {
        deleted.forEach((edge) => {
            if (edge.data?.kind !== 'model-to-role')
                return;
            const sourceNode = nodeMap.get(edge.source);
            const targetNode = nodeMap.get(edge.target);
            if (!sourceNode || !targetNode || sourceNode.type !== 'model' || targetNode.type !== 'role')
                return;
            const modelData = sourceNode.data;
            const roleData = targetNode.data;
            if (modelData.kind === 'model' && roleData.kind === 'role') {
    ...
    const deleteNode = useCallback((nodeId) => {
        ...
        const currentConfig = latestConfigRef.current;
        const onChange = onConfigChangeRef.current;
        if (!node || !currentConfig || !onChange)
            return;
        if (node.type === 'model') {
            ...
            if (data.kind === 'provider') {
                ...
                const removableNodeIds = new Set(Array.from(nodeMap.values())
                ...
                setEdges((current) => current.filter((edge) => !removableNodeIds.has(edge.source) && !removableNodeIds.has(edge.target)));
            }
        }
    ...
    const addModel = useCallback((providerId, model) => {
        if (!providerId || !model)
            return;
        const existing = nodes.find((node) => node.type === 'model' && node.data.kind === 'model' && node.data.providerId === providerId && node.data.model === model);
        if (existing)
            return;
