...
export function ProcessMonitorSidebar({
  ...
  return (
    <div className="soft-panel-subtle flex flex-col h-full text-text-main">
      {/* Sidebar Header & Tabs */}
      <div className="soft-panel-subtle flex flex-col border-b border-white/5">
        <div className="px-3 py-2 flex items-center gap-2">
            <Activity className="size-4 text-slate-400" />
            <span className="text-xs font-bold tracking-wide text-slate-200">System Monitor</span>
        ...
        <div className="grid grid-cols-4 gap-1 px-1 pb-1">
            <button
                ...
                className={`flex h-8 min-w-0 items-center justify-center gap-1 py-1.5 text-[10px] uppercase font-bold tracking-wider rounded-t-mk transition-colors ${
                    activeTab === 'pm' 
                    ? 'soft-raised text-accent border-t-2 border-accent'
         ...
         <div className={`absolute inset-0 overflow-y-auto ${activeTab === 'usage' ? 'z-10' : 'z-0 invisible'}`}>
            <div className="p-4 space-y-6">
                ...
                <div className="flex items-center justify-between">
                   <h3 className="text-xs uppercase font-bold text-slate-400 tracking-wider">Usage Stats</h3>
                   {onRefreshUsage && (
                     <button 
                ...
                {usageLoading && (
                  <div className="flex items-center justify-center py-8">
                    <RefreshCw className="size-5 text-slate-400 animate-spin" />
                    <span className="ml-2 text-sm text-slate-400">加载Usage数据...</span>
                  </div>
                )}
                   ...
                   <div className="grid grid-cols-2 gap-3">
                           <div className="bg-white/5 p-4 rounded-lg border border-white/10">
                               <div className="flex items-center gap-2 mb-2">
                                   ...
                                   <div className="text-[10px] text-slate-400 uppercase tracking-wider">Token Usage</div>
                               </div>
                               <div className="text-3xl font-mono font-bold text-slate-100">{usageStats.totals.total_tokens.toLocaleString()}</div>
                           ...
                           <div className="bg-white/5 p-4 rounded-lg border border-white/10">
                               <div className="flex items-center gap-2 mb-2">
                                   ...
                                   <div className="text-[10px] text-amber-400/70 uppercase tracking-wider">LLM Calls</div>
                               </div>
                               <div className="text-3xl font-mono font-bold text-amber-100">{usageStats.calls.toLocaleString()}</div>
                               <div className="text-[9px] text-amber-500/50 mt-1">
                ...
                {usageStats.totals.total_tokens > 0 && (
                    <div className="p-3 rounded-lg border border-amber-500/20 bg-amber-500/5">
                        <div className="text-[10px] uppercase tracking-wider text-slate-400 mb-1">Cost Estimate</div>
                        <div className="flex items-baseline gap-2">
                            <span className="text-lg font-mono text-slate-200">${(usageStats.totals.total_tokens / 1000 * 0.003).toFixed(3)}</span>
                            <span className="text-[10px] text-slate-500">≈ $3/1M tokens (Claude)</span>
                        </div>
                    </div>
                )}
                ...
                {usageStats?.by_mode && Object.keys(usageStats.by_mode).length > 0 && (
                     <div className="space-y-2">
                        <h3 className="text-xs uppercase font-bold text-slate-400 tracking-wider">Breakdown</h3>
                        <div className="space-y-1.5">
                            {Object.entries(usageStats.by_mode)
                                ...
                                .map(([mode, stats]) => {
                                    ...
                                    return (
                                    <div key={mode} className="flex items-center justify-between text-xs bg-white/5 px-3 py-2.5 rounded-lg border border-white/5 hover:border-white/10 transition-colors">
                                        <div className="flex items-center gap-2">
                                            {backend.icon}
                                            <div className="flex flex-col">
                                                <span className="font-medium text-slate-200">{backend.label}</span>
                                                <span className="text-[9px] text-slate-500">{backend.description}</span>
                                            </div>
                                        </div>
                                        <div className="flex items-center gap-3">
                                            <span className="text-slate-400 font-mono text-[10px]">{stats.calls} 次</span>
                                            <span className="text-slate-300 font-mono font-medium">{stats.total_tokens.toLocaleString()} tks</span>
                                        </div>
                                    </div>
                                    );
                                })}
                ...
                {!usageStats && !usageLoading && !usageError && (
                   <div className="flex flex-col items-center justify-center py-12 text-center">
                        <PieChart className="size-10 text-slate-600 mb-3" />
                        <div className="text-sm text-slate-500 italic">暂无Usage记录</div>
                        <div className="text-[10px] text-slate-600 mt-1">运行 PM/Director 后将显示统计数据</div>
                   </div>
                )}
            </div>
         </div>
