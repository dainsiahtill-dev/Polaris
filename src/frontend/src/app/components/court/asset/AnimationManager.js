...
// 动画状态映射配置
export const STATUS_ANIMATION_MAP = {
    offline: 'idle',
    idle: 'idle',
    thinking: 'thinking',
    executing: 'working',
 ...
 * 创建动画管理器
 */
export function createAnimationManager(asset) {
 ...
 * 切换动画状态
 */
export function transitionToAnimation(state, asset, status, config = DEFAULT_TRANSITION) {
 ...
 * React Hook: 使用角色动画
 */
export function useRoleAnimation(asset, status, isVisible = true) {
 ...
 * 更新动画混合器
 */
export function updateAnimationMixer(mixer, deltaTime) {
    mixer.update(deltaTime);
}
/**
 * 批量更新多个动画混合器
 */
export function updateAllMixers(mixers, deltaTime) {
 ...
 */
export function useAnimationFrame(mixers) {
