...
import { getFactoryRun, stopFactoryRun } from '@/services/factoryService';
...
function runLifecycleToken(run) {
...
function isTerminalFactoryRun(run) {
    return TERMINAL_FACTORY_RUN_STATUSES.has(runLifecycleToken(run));
}
export function useRoleSessionFactoryExport({ sessionId, logPrefix, }) {
    const [isExportingFactory, setIsExportingFactory] = useState(false);
    const [factoryRunEvidence, setFactoryRunEvidence] = useState({
        runId: null,
        loading: false,
        data: null,
        error: null,
    });
    const [factoryRunCancelEvidence, setFactoryRunCancelEvidence] = useState({
        runId: null,
        loading: false,
        message: null,
        error: null,
    });
    const loadFactoryRunEvidence = useCallback(async (runId, options = {}) => {
    ...
    const handleExportToFactory = useCallback(async () => {
        if (!sessionId || isExportingFactory)
            return;
    ...
    const cancelFactoryRunDisabled = !factoryRunEvidence.runId ||
