...
export function PtyDrawer({
  ...
  return (
    <Drawer open={open} onOpenChange={onOpenChange} direction="right">
      <DrawerContent className="data-[vaul-drawer-direction=right]:w-[85vw] data-[vaul-drawer-direction=right]:sm:w-[560px] data-[vaul-drawer-direction=right]:sm:max-w-[560px] soft-panel border-l">
        <DrawerHeader className="border-b border-white/10 soft-panel-subtle">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2 text-xs text-text-muted">
              <TerminalSquare className="size-4" />
              <span className="font-semibold tracking-wide">模型检阅终端</span>
            </div>
            <span className="text-[10px] uppercase tracking-widest text-text-dim">{roleLabel}</span>
          </div>
          ...
          <DrawerDescription className="text-[11px] text-text-dim">
            先启动 CLI，通过 <span className="text-accent">/models</span> 或 <span className="text-accent">/model</span>
            查阅模型，再将编号贴入下方。
          </DrawerDescription>
        </DrawerHeader>

        <div className="flex-1 overflow-hidden p-4 space-y-3">
          <div className="rounded-xl soft-inset">
            <div className="flex items-center justify-between px-3 py-2 border-b border-white/10 text-[10px] uppercase tracking-widest text-text-dim">
              ...
              <div className="flex items-center gap-1">
                ...
                <span className={status === 'error' ? 'text-red-300' : status === 'online' ? 'text-emerald-300' : 'text-text-dim'}>
                  {{ idle: '待命', connecting: '连线中', online: '在线', error: '故障', closed: '已闭' }[status]}
                </span>
              </div>
            </div>
            <div ref={handleTerminalRef} className="h-[320px] w-full" />
          </div>
          ...
          <div className="rounded-xl soft-panel-subtle p-3 space-y-2">
            <div className="flex items-center justify-between text-[10px] text-text-dim uppercase tracking-widest">
              <span>已选模型编号</span>
              <div className="flex items-center gap-2">
                {bootCommand ? (
                  <button type="button" onClick={handleBootCommand} className="text-accent hover:text-accent/80">
                    {bootCommandLabel || '启动 Codex'}
                  </button>
                ) : null}
                {autoCommand ? (
                  <button type="button" onClick={() => sendAutoCommand(true)} className="text-accent hover:text-accent/80">
                    {autoCommandLabel || '发送 /model'}
                  </button>
                ) : null}
                <button type="button" onClick={handlePaste} className="text-accent hover:text-accent/80">
                  粘贴
                </button>
              </div>
            </div>
            <input
              ...
              className="w-full soft-inset text-text-main px-3 py-2 rounded text-sm font-mono focus:outline-none focus:ring-1 focus:ring-accent/40"
            />
            {error ? <div className="text-[10px] text-red-300">{error}</div> : null}
          </div>
        </div>

        <div className="p-4 border-t border-white/10 soft-panel-subtle flex items-center justify-between">
          <button
            type="button"
          ...
          <div className="flex items-center gap-2">
            <button
              type="button"
              onClick={onSaveModel}
              className="px-3 py-1.5 text-[10px] soft-chip text-text-muted hover:text-text-main rounded"
            >
              保存模型
            </button>
            {showQuickTest ? (
              <button
                ...
                className="px-3 py-1.5 text-[10px] soft-raised text-accent rounded flex items-center gap-1"
              >
                <Sparkles className="size-3" />
