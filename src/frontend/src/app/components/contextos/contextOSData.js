 ...
 */
import { EMPTY_TELEMETRY, contextOSObservedTokens, filterEventsForRole, telemetryRoleEvents, telemetryRoleHasUsageChannel, telemetryRoleTokens, } from './contextOSTelemetry';
...
export const ROLE_DECISION_ALIASES = {
    pm: ['pm'],
    architect: ['architect'],
...
export function decisionMatchesRole(actor, roleId) {
    if (!roleId)
        return true;
...
export function safeText(value, fallback = '') {
    ...
    if (typeof value === 'string')
        return redactedJsonDisplayText(value) ?? value;
...
function isProviderRequestSnapshotCandidate(event) {
    return Boolean(event.contextSnapshotRef
        && (event.isCall
            || event.callId
            || event.hasUsage
...
function formatWindowTokens(value) {
    if (value >= 1000000) {
...
function roleDetailAsBinding(detail) {
    return {
        bindingId: null,
...
function emptyBindingBudget(roleKey, binding, index, source) {
...
function deriveRoleBindingBudgetTemplates(state, roleKey) {
    ...
    return bindings.map((binding, index) => {
        ...
        const skipped = (state.skippedBindings || []).find((item) => {
            ...
            if (!roleMatches)
                return false;
            if (item.bindingId && row.bindingId)
                return keyToken(item.bindingId) === keyToken(row.bindingId);
            if (item.providerId && row.providerId)
                return keyToken(item.providerId) === keyToken(row.providerId) && (!item.model || !row.model || keyToken(item.model) === keyToken(row.model));
...
function eventMatchesBinding(event, binding) {
    ...
    if (eventBindingId && bindingId)
        return eventBindingId === bindingId;
...
function summarizeUsageKind(events) {
...
function summarizeUsageProvenance(events) {
...
function summarizeBindingBudget(template, events, usageSource) {
    if (events.length === 0)
        return template;
    ...
    for (const event of orderedEvents) {
        ...
        if (event.hasUsage && !event.estimatedTokens)
            realProviderTokens += event.totalTokens;
        if (event.estimatedTokens || event.usageSource === 'request_estimate')
            estimatedTokens += observedTokens;
        if (event.isCall || event.hasUsage)
            calls += 1;
    }
    const latestContextSize = orderedEvents.find((event) => event.finalRequestTokenEstimate !== null || event.contextTokens !== null);
...
function deriveRoleContextWindow(state, roleKey) {
    const detail = roleDetailForWindow(state, roleKey);
    if (!detail) {
...
function phaseToActiveStage(phase, running) {
    const token = (phase || '').trim().toLowerCase();
    if (!running)
        return null;
    if (token.includes('plan'))
        return 'projection';
    if (token.includes('dispatch'))
        return 'budget';
    if (token.includes('implement') || token.includes('exec') || token.includes('code') || token.includes('cod'))
        return 'llm';
    if (token.includes('review') || token.includes('qa') || token.includes('test'))
        return 'telemetry';
    return 'working_mem';
}
...
function deriveDecisions(dialogueEvents, executionLogs) {
    const rows = [];
    dialogueEvents.slice(-14).forEach((event, index) => {
        ...
        const tone = /error|fail/i.test(kind)
            ? 'error'
...
function telemetryDecisionTone(event) {
    if (event.category === 'error')
        return 'error';
    if (event.isProjection || event.hasReceipt)
        return 'success';
    if (event.category === 'call')
        return 'info';
    return 'info';
}
function telemetryDecisionKind(event) {
    if (event.name)
        return safeText(event.name);
    if (event.isProjection)
        return 'projection';
    return safeText(event.kind, 'observation');
}
function deriveTelemetryDecisions(telemetry, limit = 12) {
...
export function buildContextOSModel(input) {
    const { usageStats, dialogueEvents, executionLogs, snapshot, llmRuntimeState, currentPhase, pmRunning, directorRunning, } = input;
    ...
    const byMode = telemetryActive
        ? Object.fromEntries(Object.entries(telemetry.byMode).map(([key, value]) => [key, { total_tokens: value.totalTokens, calls: value.calls }]))
        : usageStats?.by_mode ?? {};
    ...
    const roleWindowByKey = Object.fromEntries(ROLE_DEFINITIONS.map((role) => [role.key, deriveRoleContextWindow(llmRuntimeState, role.key)]));
    ...
    const contextWindowSource = configuredRoleWindows.length > 0 ? 'binding' : 'unknown';
    ...
    const impliedStage = telemetryActive && latestEvent
    ...
    const stateFor = (id) => {
        if (llmBlocked && (id === 'llm' || id === 'prompt'))
            return 'blocked';
        if (id === 'role_signal' && blockedRoles.size > 0)
            return 'blocked';
        if (activeStageId === id)
            return 'active';
        return 'idle';
    ...
    const budgetDenominator = Math.max(totalTokens, directPromptTokens + cacheCreationTokens + cacheReadTokens + completionTokens + toolTokens + reasoningTokens + audioTokens, 0);
    const rawBudget = [
    ...
    const byModeSlices = Object.entries(byMode)
    ...
    const eventTypes = telemetryActive
    ...
    const roleState = (roleKey) => {
        if (blockedRoles.has(roleKey))
            return 'blocked';
        if (roleKey === 'pm' && pmRunning)
            return 'active';
        if (roleKey === 'director' && directorRunning)
            return 'active';
        // 真实遥测里该角色产生过事件 → 活动（如一次角色对话）。
        if (telemetryActive && telemetryRoleEvents(telemetry, roleKey) > 0)
            return 'active';
        return 'idle';
    };
    const roleTokens = (roleKey) => {
    ...
    function buildRoleInternalContext(role) {
        ...
        const lastContextSize = roleEvents.find((event) => event.finalRequestTokenEstimate !== null || event.contextTokens !== null);
        ...
        const roleWindow = roleWindowByKey[role.key] ?? { tokens: null, label: '窗口未知', detail: 'LLM status 未提供角色绑定窗口', source: 'unknown', provider: null, model: null };
        ...
        const usageEvents = roleEvents.filter((event) => event.isCall || event.hasUsage || event.finalRequestTokenEstimate !== null || event.contextTokens !== null);
        const bindingBudgets = bindingTemplates.map((template) => {
            ...
            if (matched.length > 0)
                return summarizeBindingBudget(template, matched, 'matched');
        ...
        if (bindingTemplates.length > 1) {
            const unassignedEvents = usageEvents.filter((event) => !bindingTemplates.some((template) => eventMatchesBinding(event, template)));
    ...
    const workers = telemetryActive
        ? Object.values(telemetry.byWorker)
            .map((agg) => {
            // 该 worker 命中的最近一次 context snapshot ref（按 epoch 倒序扫）。
            const lastSnapshot = telemetry.events.find((event) => event.workerId === agg.workerId && event.contextSnapshotRef);
            return {
 ...
 */
export function summarizeRoleContextState(ctx) {
    const title = ctx.title || ctx.roleId || '该角色';
    const errorCount = ctx.events.reduce((count, event) => count + (event.category === 'error' ? 1 : 0), 0);
    // 1. 受阻：优先暴露错误，避免被「有调用」掩盖。
    if (ctx.state === 'blocked' && errorCount > 0) {
        return {
            tone: 'blocked',
            headline: `${title} 观测到 ${errorCount} 条错误事件，已受阻`,
            detail: `请查看下方事件流定位错误${ctx.calls > 0 ? `；此前已调用 ${ctx.calls} 次` : '；尚未成功调用模型'}`,
        };
    }
    // 2. 完全无事件。
    if (ctx.eventCount === 0) {
