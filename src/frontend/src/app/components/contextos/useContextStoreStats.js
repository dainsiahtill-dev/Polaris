...
import { parseContextStoreStatsResponse, } from './contextosStoreStats';
/** 单次请求超时。后端 admin stats 是 cheap stat snapshot，10s 足矣。 */
const REQUEST_TIMEOUT_MS = 10000;
function readErrorPayload(body) {
    try {
        const parsed = JSON.parse(body);
        if (parsed && typeof parsed === 'object') {
            const obj = parsed;
            const detail = obj.detail && typeof obj.detail === 'object' ? obj.detail : null;
            return {
