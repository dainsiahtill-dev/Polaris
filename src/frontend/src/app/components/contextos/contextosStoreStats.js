...
export const STATS_STATUS_COLOR = {
...
/** 状态 → 展示标签（中文）。 */
export const STATS_STATUS_LABEL = {
    ok: '健康',
    warning: '接近上限',
    critical: '超限',
    disabled: '已禁用',
...
export function formatRelativeSeconds(epochSeconds, nowMs = Date.now()) {
...
export function parseContextStoreStatsResponse(payload) {
    if (!payload || typeof payload !== 'object')
        return null;
    const obj = payload;
    const configRaw = obj.config && typeof obj.config === 'object' ? obj.config : {};
        ...
        last_sweep_report: reportRaw
            ? {
                ...
                triggers: Array.isArray(reportRaw.triggers)
                    ? reportRaw.triggers.filter((s) => typeof s === 'string')
                    : null,
            }
            : null,
    };
...
export function deriveNextSweepAt(stats) {
    const interval = stats.config.sweep_min_interval_seconds;
    if (typeof interval !== 'number' || interval <= 0)
        return null;
    if (!Number.isFinite(stats.last_sweep_at) || stats.last_sweep_at <= 0)
        return null;
    return stats.last_sweep_at + interval;
}
/** 派生 UI 展示用的 "X 天前最旧" 年龄（秒）。oldest_mtime 非有限 → null。 */
export function deriveOldestAgeSeconds(stats, nowMs = Date.now()) {
    if (typeof stats.oldest_mtime !== 'number' || !Number.isFinite(stats.oldest_mtime))
        return null;
