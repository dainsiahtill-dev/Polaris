...
function normalizeRoleState(value: unknown): RoleState {
  const token = String(value || '').trim().toLowerCase();
  if (!token) return 'idle';
  if (token === 'analyzing') return 'analyzing';
  if (token === 'planning' || token === 'intake' || token === 'docs_check' || token === 'architect') return 'planning';
  if (token === 'verification' || token === 'qa_gate') return 'verification';
  if (token === 'completed' || token === 'done' || token === 'success' || token === 'handover') return 'completed';
  if (token === 'failed' || token === 'error' || token === 'cancelled' || token === 'canceled') return 'failed';
...
function normalizePhase(value: unknown): Phase | null {
  ...
  if (
    ...
    return token as Phase;
  }
  if (token === 'idle') return 'pending';
  if (token === 'executing' || token === 'tool_running' || token === 'llm_calling') return 'implementation';
  if (token === 'analyzing') return 'planning';
  if (token === 'error' || token === 'canceled') return 'failed';
  return null;
}
...
function normalizeTaskState(value: unknown): TaskState {
  ...
  if (token === 'claimed') return 'claimed';
  if (token === 'in_progress' || token === 'running') return 'in_progress';
  if (token === 'completed' || token === 'done' || token === 'success') return 'completed';
...
function normalizeWorkerState(value: unknown): RuntimeWorkerState['state'] {
  const token = String(value || '').trim().toLowerCase();
  if (token === 'claimed') return 'claimed';
  if (token === 'in_progress' || token === 'busy' || token === 'running') return 'in_progress';
  if (token === 'completed' || token === 'success') return 'completed';
  if (token === 'failed' || token === 'error') return 'failed';
  return 'idle';
}

function mapSeverity(level: LogEntry['level']): EventSeverity {
...
export function useRoleState(role: RoleType): RuntimeRoleView {
...
export function useSummary(): RuntimeSummary {
...
export function useRecentEvents(count: number = 50): RuntimeEventV2[] {
  const events = useRuntimeEvents();
  return useMemo(() => events.slice(-count), [events, count]);
}

export function useEventsBySeverity(severity: EventSeverity): RuntimeEventV2[] {
  const events = useRuntimeEvents();
  return useMemo(() => events.filter((event) => event.severity === severity), [events, severity]);
}

export function useBlockedTasks(): BlockedRuntimeTask[] {
  const tasks = useTasks();
  return useMemo(() => tasks.filter((task): task is BlockedRuntimeTask => task.state === 'blocked'), [tasks]);
}

export function useRootTasks(): RuntimeTaskNode[] {
  const tasks = useTasks();
  return useMemo(() => tasks.filter((task) => task.level === 1), [tasks]);
}

export function useChildTasks(parentId: string): RuntimeTaskNode[] {
  const tasks = useTasks();
