 ...
 */
import { isRuntimeProjectionPayload, } from "./projection";
// ============================================================================
...
function deriveOutstandingTasks(metrics, completedTasks, failedTasks) {
    const explicit = metricValue(metrics, "active_tasks", "tasks_active", "tasks_running", "running_tasks", "queue_depth", "tasks_pending", "pending_tasks");
...
function normalizeDirectorStateToken(value, runningHint) {
    ...
    if (["failed", "failure", "error", "failed_platform", "failed_artifact", "blocked_with_reason"].includes(state) ||
...
function collectRuntimeStatusFields(response) {
    if (!response || typeof response !== "object")
        return [];
    const obj = response;
    return RUNTIME_STATUS_EVENT_FIELDS.filter((field) => field in obj);
}
function attachProjectionProvenance(projection, provenance) {
    return {
        ...projection,
 ...
 */
export function normalizeRuntimeProjection(response) {
    ...
    if (isCanonicalProjection(response)) {
        ...
        return attachProjectionProvenance(canonical, createProjectionProvenance({
            source: "canonical",
            transformed: false,
            receivedAt,
 ...
 */
function isCanonicalProjection(response) {
    return isRuntimeProjectionPayload(response);
}
/**
 * Check whether input is a runtime.v2 status event shape.
 */
function isRuntimeStatusEvent(response) {
    if (!response || typeof response !== "object")
        return false;
    const event = response;
    return ((event.pm_status !== undefined && (event.pm_status === null || typeof event.pm_status === "object")) ||
 ...
 */
function normalizePMStatus(event) {
 ...
 */
function normalizeDirectorStatus(event) {
    ...
    if (directorNested && typeof directorNested === 'object') {
        ...
        const stateToken = normalizeDirectorStateToken(directorNested.execution_state || directorNested.state || serviceStatus?.execution_state || serviceStatus?.state, Boolean(directorNested.running) || Boolean(serviceStatus?.is_running) || activeTasks > 0);
 ...
 */
function normalizeWorkflowStatus(event) {
 ...
 */
function normalizeEngineStatus(event) {
 ...
 */
function normalizePMPhase(phase) {
 ...
 */
function normalizeDirectorPhase(phase) {
...
function normalizePriority(priority) {
    ...
    return validPriorities.includes(normalized)
        ? normalized
        : undefined;
}
/**
...
function normalizeHealthStatus(health) {
    ...
    if (normalized === "healthy" || normalized === "ok" || normalized === "good")
        return "healthy";
    if (normalized === "degraded" || normalized === "warning")
        return "degraded";
    if (normalized === "unhealthy" || normalized === "error" || normalized === "bad")
        return "unhealthy";
    return "unknown";
}
/**
 ...
 */
export function createEmptyProjection(reason = "empty_runtime_projection") {
    const generatedAt = new Date().toISOString();
    return attachProjectionProvenance({
        pm: null,
        director: null,
 ...
 */
export function mergeProjections(base, update) {
 ...
 */
export function createPartialProjection(partial) {
