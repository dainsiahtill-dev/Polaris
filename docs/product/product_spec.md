...
| 官职 | 角色 | 职责 | CLI 入口 |
|------|------|------|----------|
| 尚书令 | **PM** | 选方向、拆任务、写验收标准 | `scripts/pm/cli.py` |
| 中书令 | **Architect** | 架构设计、技术选型 | `role_agent/architect_cli.py` |
| 工部尚书 | **Chief Engineer** | 技术分析、代码审查 | `role_agent/chief_engineer_cli.py` |
| 工部侍郎 | **Director** | 取证、改动、验证 | `scripts/director/cli_thin.py` |
| 门下侍中 | **QA** | 质量审查、测试验证 | Factory/Pipeline 集成 |
| 探子 | **Scout** | 并发只读访问层 (探索/搜索/总结) | 即将添加 (sub-agent) |
