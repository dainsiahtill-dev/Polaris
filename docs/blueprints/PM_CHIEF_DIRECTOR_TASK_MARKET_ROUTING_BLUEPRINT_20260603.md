...
Date: 2026-06-03
Status: Superseded by full-chain-only governance
...
- Historical shadow dispatch published tasks directly to `pending_exec`; this is now retired and must not be restored.
...
Current governance requires a single runtime path: `PM -> ChiefEngineer -> Director`. PM cannot send work directly to
Director. Every task must first publish to `pending_design` and receive ChiefEngineer blueprint/handoff evidence before it
can reach `pending_exec`.
...
PM task contract
  |
  `-- governed task -> task_market.pending_design
                                |
                                v
                           ChiefEngineer
                                |
...
PM dispatch records a single governed route for each task:
...
Historical `direct_to_director`, `pending_exec`, `director`, `direct`, and `execution` route hints are compatibility
aliases only. Runtime normalizes them to `chief_blueprint_required` / `pending_design`.
...
1. Unit test PM dispatch routing:
   - every PM task publishes `pending_design`
   - legacy direct route hints normalize to `chief_blueprint_required`
